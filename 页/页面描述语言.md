'''页面描述语言'''（简称 PDL）是一种在较高层次上描述实际输出结果[[位图|位图]]的描述页面内容的语言。 一些页面描述语言并不完备，很难称为是一种[[编程语言|编程语言]]，但是作为页面描述语言之一的 [[PostScript|PostScript]] 则是一个非常完备的编程语言。

==列表==
*[[Advanced_Function_Presentation|AFP]] (Advanced Function Presentation)
*[[DVI_file_format|DVI]] (stands for DeVice Independent)
*[[Eltron_Programming_Language|EPL]] (Eltron Programming Language)
*[[ESC/P2|ESC/P2]]
*[[IPDS|IPDS]] (Intelligent Printer Data Stream)
*Metacode
*[[PCL|PCL]] (Printer Command Language)
*[[PDF|PDF]] (Portable Document Format)
*[[PLT|PLT]](HPGL)
*[[PostScript|PostScript]]
*[[IBM_Personal_Printer_Data_Stream|PPDS]] (Personal Printer Data Stream).
*[[Scalable_Vector_Graphics|SVG]] (Scalable Vector Graphics).
*[[XSL-FO|XSL-FO]] (XSL Formating Objects)
*[[XML纸张规范|XPS]]（XML Paper Specification）

==参见==
*[[置标语言|置标语言]]

==外部链接==
*[http://www.undocprint.org/formats/page_description_languages Undocumented Printing]
*[http://www.nefec.org/UPM/ilang.htm Ultimate Printer Manual]
*[http://www.iana.org/assignments/printer-language-numbers Printer MIB Interpreters Language Families]
*[http://www-306.ibm.com/software/webservers/hostondemand/library/infocentergafinal/hod/en/doc/hostprint/hostprintref.html  Host Printing Reference] Contains example for PPDS.
*[http://knowledgebase.lexmark.com/pubfiles/files/PATTLNK_2182005_1515-PPDS%20Escape%20Codes.pdf PPDS Escape codes]

[[Category:页面描述语言|Category:页面描述语言]]

{{compu-lang-stub}}