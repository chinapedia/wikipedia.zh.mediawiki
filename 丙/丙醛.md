{{Chembox
|   Name = 丙醛
|   ImageFile = Propanal-skeletal.png
|   ImageName = 丙醛结构
|   IUPACName = Propionaldehyde
|   SystematicName = Propanal
|   OtherNames = 
|Section1={{Chembox Identifiers
| UNII_Ref = {{fdacite|correct|FDA}}
| UNII = AMJ2B4M67V
| ChEBI_Ref = {{ebicite|correct|EBI}}
| ChEBI = 17153
| SMILES = CCC=O
| PubChem = 527
| InChI = 1/C3H6O/c1-2-3-4/h3H,2H2,1H3
| InChIKey = NBBJYMSMWIIQGU-UHFFFAOYAZ
| ChEMBL_Ref = {{ebicite|correct|EBI}}
| ChEMBL = 275626
| StdInChI_Ref = {{stdinchicite|correct|chemspider}}
| StdInChI = 1S/C3H6O/c1-2-3-4/h3H,2H2,1H3
| StdInChIKey_Ref = {{stdinchicite|correct|chemspider}}
| StdInChIKey = NBBJYMSMWIIQGU-UHFFFAOYSA-N
| CASNo_Ref = {{cascite|correct|CAS}}
| CASNo = 123-38-6
| ChemSpiderID_Ref = {{chemspidercite|correct|chemspider}}
| ChemSpiderID = 512
| UNNumber = 1275
| RTECS = 
  }}
|Section2 = {{Chembox Properties
|   Formula = C<sub>3</sub>H<sub>6</sub>O
|   MolarMass = 58.080 
|   Appearance = 无色有刺激性气味液体
|   Density = 0.81 g/cm<sup>3</sup>
|   Solubility = 20 g/100 mL
|   MeltingPt = −81 °C (192 K)
|   BoilingPt = 46-50 °C (321 K)
|   Viscosity = 0.6 [[泊_(单位)|cP]], 20°C
  }}
| Section3 = {{Chembox Structure
|   MolShape = C<sub>1</sub>, O: sp<sup>2</sup>
C<sub>2</sub>, C<sub>3</sub>: sp<sup>3</sup>
|   Dipole = 2.52 [[德拜|D]]
  }}
| Section7 = {{Chembox Hazards
|   ExternalMSDS =
|   EUClass = 高度易燃 ('''F''')<br />刺激性 ('''Xi''')
|   NFPA-H = 2
|   NFPA-F = 3
|   NFPA-R = 2
|   RPhrases = {{R11}}, {{R36/37/38}}
|   SPhrases = {{S9}}, {{S16}}, {{S29}}
|   FlashPt = −26 °C
|   Autoignition = 175 °C
  }}
| Section8 = {{Chembox Related
|   Function = [[醛|醛]]
|   OtherFunctn = [[甲醛|甲醛]]<br />[[乙醛|乙醛]]<br />[[丁醛|丁醛]]
  }}
}}

'''丙醛'''是三个碳的[[醛|醛]]，分子式为CH<sub>3</sub>CH<sub>2</sub>CHO。它是[[丙酮|丙酮]]的同分异构体，室温下为无色液体，略微带有刺激性的水果气味。

丙醛主要通过金属催化剂存在下，混合[[合成气|合成气]]和[[乙烯|乙烯]]，发生[[加氢甲酰化|加氢甲酰化]]反应制得：
:CO + H<sub>2</sub> + C<sub>2</sub>H<sub>4</sub> → CH<sub>3</sub>CH<sub>2</sub>CHO

== 用途 ==
丙醛主要用于与[[甲醇|甲醇]]发生[[缩合反应|缩合反应]]制取[[三羟甲基乙烷|三羟甲基乙烷]]，进一步制取[[醇酸树脂|醇酸树脂]]。

丙醛与[[叔丁胺|叔丁胺]]缩合得到CH<sub>3</sub>CH<sub>2</sub>CH=N-''t''-Bu，是[[有机合成|有机合成]]中常用的三碳试剂。用[[LDA|LDA]]对这个[[亚胺|亚胺]]去质子化得到CH<sub>3</sub>CHLi<sub>2</sub>CH=N-''t''-Bu，然后与[[醛|醛]]缩合。<ref>Peralta, M. M.
"Propionaldehyde t-Butylimine" in Encyclopedia of Reagents for Organic Synthesis (Ed: L. Paquette) 2004, J. Wiley & Sons, New York. DOI: 10.1002/047084289.</ref>

== 参考资料 ==
<references/>

{{Organic-compound-stub}}

[[Category:醛|Category:醛]]
[[Category:空气污染物|Category:空气污染物]]
[[Category:三碳有机物|Category:三碳有机物]]
[[Category:缺少物质图片的化学品条目|Category:缺少物质图片的化学品条目]]