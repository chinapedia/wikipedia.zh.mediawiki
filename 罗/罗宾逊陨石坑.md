{{lunar crater data|
 image=Normal Robinson LO-IV-164 LTVT.jpg|240px|
 caption=[[月球轨道器4号|月球轨道器4号]]拍摄的图像|
 latitude=59.06|
 N_or_S=N|
 longitude=46.03|
 E_or_W=W|
 diameter=24.09公里|
 depth=1.88公里|
 colong=46|
 eponym=约翰·托·罗·罗宾逊}}
'''罗宾逊陨石坑'''（Robinson）是[[月球正面|月球正面]]位于[[冷海|冷海]]西北岸附近的一座小[[撞击坑|撞击坑]]，其名称取自[[爱尔兰|爱尔兰]][[天文学家|天文学家]]"约翰·托马斯·罗姆尼·罗宾逊"(John Thomas Romney Robinson，1792年-1882年)，1935年被[[国际天文学联合会|国际天文学联合会]]正式批准接受。

==描述== 
[[image:J._Herschel_-_LROC_-_WAC.jpg|400px]]拍摄。]] 
[[image:M114259768R_Robinson.png|240px]]拍摄的照片，图像宽度620米，分辨率52厘米/像素。]] 
该陨坑西南偏南靠近[[索思环形山|索思环形山]]，东北毗邻[[约·赫歇尔环形山|约·赫歇尔环形山]]，[[赫瑞鲍陨石坑|赫瑞鲍陨石坑]]及[[哈尔帕卢斯陨石坑|哈尔帕卢斯陨石坑]]分别位于它的东面与东南偏南，而东南和西南分别坐落了[[冷海|冷海]]和[[露湾|露湾]]<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_11_lo.pdf Crater Robinson on the map LAC-11.]</ref>。

该陨坑中心[[月面坐标|月面坐标]]为{{Coord|59.06|N|46.03|W|globe:Moon|nogoogle=1|noosm=1}}，直径24.09公里<ref name="IAU">[http://planetarynames.wr.usgs.gov/Feature/5163 Directory of the International Astronomical Union]</ref>，深度约1.88公里<ref name="LICD"/>。

罗宾逊陨石坑外观轮廓接近圆形，只略受轻微的撞击侵蚀，坑壁仍保持峻峭清晰，东北侧切入了一串短[[链坑|链坑]]，东南侧嵌入有一座小陨坑。沿内壁显示有[[山崩|崩塌]]或滑坡的痕迹，并在坡底，特别是西半侧形成[[岩屑堆|岩屑堆]]。该陨坑坑壁最大高出周边地形840米<ref name=LICD>Lunar Impact Crater Database</ref>，内部容积约370公里<sup>3</sup><ref name="LICD"/>。坑底表面地形崎岖，在东半区坐落了一座小撞击坑。{{clear}}

==参考文献==
{{reflist}}

==另请参阅==
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | date = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | date = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | date = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| year = 1971| pmid = | pmc =| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | date = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | date = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | date = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | date = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revised
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | date = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | date = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==外部链接==
*[http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Robinson%7C0 月球数码摄影图集]
*[http://the-moon.wikispaces.com/Robinson 维基月球在线解说-罗宾逊陨石坑.]
* [http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月.]

[[Category:月球环形山|R]]