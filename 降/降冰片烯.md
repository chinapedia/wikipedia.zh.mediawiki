{{chembox
| verifiedrevid = 450599059
| Reference = <ref>[https://fscimage.fishersci.com/msds/66955.htm Norbornene MSDS]</ref>
| ImageFileL1 = 2-norbornene 200.svg
| ImageSizeL1 = 100px
| ImageFileR1 = Norbornene2.png
| ImageSizeR1 = 120px
| ImageFile2 = Norbornene3.png
| ImageSize2 = 150px
| IUPACName = Bicyclo[2.2.1]hept-2-ene
| OtherNames = Norbornylene<br>Norcamphene
| Section1 = {{Chembox Identifiers
| Abbreviations =
|  PubChem = 10352
|  ChemSpiderID = 9925
|  InChI = 1/C7H10/c1-2-7-4-3-6(1)5-7/h1-2,6-7H,3-5H2
|  InChIKey = JFNLZVQOOSMTJK-UHFFFAOYAB
|  StdInChI = 1S/C7H10/c1-2-7-4-3-6(1)5-7/h1-2,6-7H,3-5H2
|  StdInChIKey = JFNLZVQOOSMTJK-UHFFFAOYSA-N
| CASNo_Ref = {{cascite|correct|CAS}}
| CASNo = 498-66-8
| EINECS = 207-866-0
| SMILES = C1/C2=C\CC1CC2
| RTECS =
| MeSHName =
| ChEBI_Ref = {{ebicite|correct|EBI}}
| ChEBI =
| KEGG_Ref = {{keggcite|correct|kegg}}
| KEGG =
| ATCCode_prefix =
| ATCCode_suffix =
| ATC_Supplemental =}}
| Section2 = {{Chembox Properties
| C=7|H=10
| Appearance = White solid
| Density =
| MeltingPtCL = 42
| MeltingPtCH = 46
| BoilingPtC = 96
| Boiling_notes =
| Solubility =
| SolubleOther =
| Solvent =
| pKa =
| pKb = }}
| Section7 = {{Chembox Hazards
| EUClass =
| EUIndex =
| MainHazards =
| NFPA-H = 2
| NFPA-F = 3
| NFPA-R = 1
| NFPA-O =
| RPhrases =
| SPhrases =
| RSPhrases =
| FlashPt = −15 °C
| Autoignition =
| ExploLimits =
| PEL = }}
}}

降冰片烯（Norbornene），也称降莰烯（norcamphene），是一种橋環烴，为白色固體，有刺激性酸味。該分子由環己烯環和亞甲基橋（在C-3和C-6之間）组成，其中的雙鍵引起了顯著的環張力和反應活性。

降冰片烯由環戊二烯和乙烯的[[狄尔斯-阿尔德反应|狄尔斯-阿尔德反应]]合成；其衍生物也是类似<ref>{{OrgSynth |author=Paul Binger, Petra Wedemann, and Udo H. Brinker |title=Cyclopropene: A New Simple Synthesis and its Diels-Alder Reaction with Cyclopentadiene |collvol=10 |collvolpages=231 |prep=v77p0254}}</ref><ref>{{OrgSynth |author=Masaji Oda, Takeshi Kawase, Tomoaki Okada, and Tetsuya Enomoto |title=2-Cyclohexene-1,4-dione |collvol=9 |collvolpages=186 |prep=cv9p0186}}</ref>。相关的双环化合物包括含有兩個雙鍵的{{le|降冰片二烯|norbornadiene}}，以及完全飽和的[[降冰片烷|降冰片烷]]。

降冰片烯经过酸催化水合反應，得到降冰片醇。研究非經典離子的化學家對此反應有很大的興趣。

==用途==
降冰片烯的用途不像乙烯等商品化学品那么广泛。它可以用作药物，杀虫剂，特种香料等有机物合成的骨架原料。它还可以与乙烯反应生成环烯烃共聚物（COC）。

降冰片烯常常在過渡金屬催化中被用來<ref>{{cite journal |author1=Thansandote, Praew |author2=Chong, Eugene |author3=Feldmann, Kai-Oliver |author4=Lautens, Mark |title=Palladium-Catalyzed Domino C−C/C−N Coupling Using a Norbornene Template: Synthesis of Substituted Benzomorpholines, Phenoxazines, and Dihydrodibenzoxazepines|journal=The Journal of Organic Chemistry|date=21 May 2010|year=2010|volume=75|issue=10|pages=3495–3498|doi=10.1021/jo100408p|url=http://pubs.acs.org/doi/abs/10.1021/jo100408p|pmid=20423091}}</ref>影響親電子的過渡金屬的遷移；它也可用作牺牲消耗的质子受体。在Catellani反应中它起到邻位导向的作用。

==聚降冰片烯==
降冰片烯在的開環复分解聚合（ROMP）反应中是重要的烯單體，例如经典的[[格拉布催化劑|格拉布催化劑]]案例。聚降冰片烯具有高玻璃化轉變溫度和高的光學清晰度。<ref>Mol, J. C. ''Journal of Molecular Catalysis A: Chemical'' '''2004''', ''213'' (2), 39-45.</ref>

[[Image:polynorbornene.svg|Norbornene ROMP]]

除了ROMP聚合，降冰片烯單體也經過乙烯基加成聚合。

聚降冰片烯主要用於抗震動（鐵路，建築，工業），防衝擊（個人防護設備，製鞋配件，保險槓）和改善抓地力（玩具輪胎，賽車輪胎，傳輸系統，傳輸系統，複印機，釣魚線等）。

亞乙基降冰片烯是一个相關的單體，衍生自環戊二烯和丁二烯。

==參考==
{{Reflist}}

[[Category:单体|Category:单体]]
[[Category:环烯烃|Category:环烯烃]]