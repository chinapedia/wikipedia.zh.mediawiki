{{Chembox
| verifiedrevid = 399424770
| ImageFile = HHTDD_structure.png
| ImageSize = 150px
| IUPACName = 2,6-dioxo-1,3,4,5,7,8-hexanitrodecahydro-1H,5H-diimidazo[4,5-b:4',5'-e]pyrazine
| IUPACNameZh = 2,6-氧代-1,3,4,5,7,8-六硝基十氢化-1H,5H-二咪唑并[4,5-b:4',5'-e]吡嗪
| OtherNames = 
| Section1 = {{Chembox Identifiers
|   CASNo = 
|   PubChem = 
|   SMILES = O=C2N([N](=O)=O)C3C(N2[N](=O)=O)N([N](=O)=O)C1C(N3[N](=O)=O)N([N](=O)=O)C(=O)N1[N](=O)=O
|  ChemSpiderID_Ref = {{chemspidercite|correct|chemspider}}
| ChemSpiderID = 23078717
|  StdInChI_Ref = {{stdinchicite|correct|chemspider}}
| StdInChI = 1S/C6H4N12O14/c19-5-9(15(25)26)1-2(10(5)16(27)28)8(14(23)24)4-3(7(1)13(21)22)11(17(29)30)6(20)12(4)18(31)32/h1-4H
| InChI=1/C6H4N12O14/c19-5-9(15(25)26)1-2(10(5)16(27)28)8(14(23)24)​4-3(7(1)13(21)22)11(17(29)30)6(20)12(4)18(31)32/h1-4H
| InChIKey = ZFBXJPJQJKATGM-UHFFFAOYAE
| StdInChIKey_Ref = {{stdinchicite|correct|chemspider}}
| StdInChIKey = ZFBXJPJQJKATGM-UHFFFAOYSA-N 
  }}
| Section2 = {{Chembox Properties
|   Formula = C<sub>6</sub>H<sub>4</sub>N<sub>12</sub>O<sub>14</sub>
|   MolarMass = 468.168
|   Appearance = 
|   Density = 
|   MeltingPt = 
|   BoilingPt = 
|   Solubility = 
  }}
| Section3 = {{Chembox Hazards
|   MainHazards = 
|   FlashPt = 
|   Autoignition = 
  }}
| Section6 = {{Chembox Explosive
| ShockSens = 
| FrictionSens = 
| ExplosiveV = 9700 ms<sup>-1</sup>
  }}
}}

'''六硝基六氮杂三环十二烷二酮'''，简称'''HHTDD'''，是一种威力巨大但对湿气极敏感的[[炸药|炸药]]。它本质上与环状硝胺笼状化合物，例如[[六硝基六氮杂异伍兹烷|六硝基六氮杂异伍兹烷]](CL-20)类似。它爆炸力巨大，[[爆速|爆速]]甚至超过著名的CL-20。它即使在痕量水存在下也能快速分解，使得实际应用的可能性很低。<ref>{{cite journal|doi =10.1021/jo00010a043|title =Facile synthesis and nitration of cis-syn-cis-2,6-dioxodecahydro-1H,5H-diimidazo[4,5-b:4',5'-e]pyrazine|year =1991|last1 =Vedachalam|first1 =Murugappa|last2 =Ramakrishnan|first2 =Vayalakkavoor T.|last3 =Boyer|first3 =Joseph H.|last4 =Dagley|first4 =Ian J.|last5 =Nelson|first5 =Keith A.|last6 =Adolph|first6 =Horst G.|last7 =Gilardi|first7 =Richard|last8 =George|first8 =Clifford|last9 =Flippen-Anderson|first9 =Judith L.|journal =The Journal of Organic Chemistry|volume =56|issue =10|pages =3413–3419}}</ref>

在浓[[盐酸|盐酸]]中，用1,4-甲酰基-2,3,5,6-四甲基哌嗪与[[尿素|尿素]]反应，生成2,4,6,8,10,12-六氮杂三环[7.3.0.0<sup>3,7</sup>]十二烷-5,11-二酮二盐酸盐水合物。然后进行硝化即可得到产物HHTDD。<ref>{{cite journal|author=鲁鸣久|title=一种新颖的高能炸药: 六硝基六氮杂三环十二烷二酮|journal=火炸药学报|year=2000|issue=1|pages=P23-24|accessdate=2011-09-25}}</ref>

[[File:HHTDD-synthesis.png|600px]]

==参见==
*[[2,4,6-三(三硝基甲基)-1,3,5-三嗪|2,4,6-三(三硝基甲基)-1,3,5-三嗪]]
*[[4,4'-二硝基-3,3'-氧化偶氮呋咱|4,4'-二硝基-3,3'-氧化偶氮呋咱]] (DDF)
*[[七硝基立方烷|七硝基立方烷]]
*[[八硝基立方烷|八硝基立方烷]]

==参考资料==
{{reflist}}

[[Category:炸藥|Category:炸藥]]
[[Category:硝胺|Category:硝胺]]