{{NoteTA|G1=Chemistry}}
{{chembox
| verifiedrevid = 477184191
|   Reference=<ref>''[[Merck_Index|Merck Index]]'', 11th Edition, '''7929'''</ref>
|   Name = 偏三甲苯
|   ImageFileL1 = 1,2,4-Trimethylbenzene.svg
|   ImageSizeL1 = 105px
|   ImageNameL1 = 结构式
|   ImageFileR1 = 1,2,4-Trimethylbenzene-3D-balls.png
|   ImageSizeR1 = 115px
|   ImageNameR1 = 球棍模型
|   ImageName = 1,2,4-Trimethylbenzene
|   IUPACName = 1,2,4-Trimethylbenzene
|   OtherNames = Pseudocumene,<br />Asymmetrical trimethylbenzene,<br />psi-cumene
| Section1 = {{Chembox Identifiers
|   ChemSpiderID_Ref = {{chemspidercite|correct|chemspider}}
| ChEBI_Ref = {{ebicite|correct|EBI}}
| ChEBI = 34039
| ChemSpiderID = 6977
| KEGG_Ref = {{keggcite|correct|kegg}}
| KEGG = C14533
| InChI = 1/C9H12/c1-7-4-5-8(2)9(3)6-7/h4-6H,1-3H3
| InChIKey = GWHJZXXIDMPWGX-UHFFFAOYAF
| StdInChI_Ref = {{stdinchicite|correct|chemspider}}
| StdInChI = 1S/C9H12/c1-7-4-5-8(2)9(3)6-7/h4-6H,1-3H3
| StdInChIKey_Ref = {{stdinchicite|correct|chemspider}}
| StdInChIKey = GWHJZXXIDMPWGX-UHFFFAOYSA-N
| CASNo_Ref = {{cascite|correct|CAS}}
| CASNo = 95-63-6
|   SMILES = c1c(ccc(c1C)C)C
}}
| Section2 = {{Chembox Properties
|   Formula = C<sub>9</sub>H<sub>12</sub>
|   MolarMass = 120.19 g/mol
|   Density = 0.8761 g/cm³
|   MeltingPt = -43.78 °C
|   BoilingPt = 169-171 °C
  }}
| Section3 = {{Chembox Hazards
|   EUClass = 有害 (Xn); 对环境有危害 (N)
|   ExternalMSDS = [http://www.sigmaaldrich.com/catalog/product/aldrich/t73601?lang=en&region=US Sigma-Aldrich MSDS]}}
| Section4 = {{Chembox Related
|  Function = 
|  OtherAnions = 
|  OtherCations = 
|  OtherCpds = [[邻三甲苯|邻三甲苯]]，[[均三甲苯|均三甲苯]]
|  OtherFunctn =  }}
}}

'''偏三甲苯'''（'''1,2,4-Trimethylbenzene'''）是一种无色液体，化学式C<sub>9</sub>H<sub>12</sub>，是[[三甲苯|三甲苯]]三种[[同分异构体|同分异构体]]之一。 偏三甲苯存在于[[煤焦油|煤焦油]]和石油（约3％），几乎不溶于水，溶于乙醇，乙醚和苯；易燃，具有强烈气味。
在工业上，偏三甲苯从石油分馏的[[碳九芳烃|碳九芳烃]]馏分中分离提取，该馏分中含有大约40％的偏三甲苯。
==应用==
偏三甲苯溶解在矿物油中应用于液态[[闪烁体|闪烁体]]，也同于生产消毒剂，染料，香料，和树脂。偏三甲苯另一个主要的用途是作汽油添加剂。<ref>{{cite web 
| accessdate=2008-01-28
| format=text
| publisher=美国环境保护局
| date=1994-08-01
| url=http://www.epa.gov/chemfact/s_trimet.txt
| title=Chemical Summary for 1,2,4-Trimethylbenzene}}</ref>
==参考资料==
{{reflist}}
{{芳香族化合物}}
{{DEFAULTSORT:Trimethylbenzene, 1,2,4-}}
[[Category:烷基苯|Category:烷基苯]]