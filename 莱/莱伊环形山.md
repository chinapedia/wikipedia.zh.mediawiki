{{lunar crater data|
 image=Normal ley.jpg|240px|
 caption=[[月球勘测轨道飞行器|月球勘测轨道飞行器]]拍摄的图像|
 latitude=42.11|
 N_or_S=N|
 longitude=154.84|
 E_or_W=E|
 diameter=81.1公里|
 depth=2.8公里|
 colong=205|
 eponym= 威利·莱伊}}
'''莱伊环形山'''（Ley）是位于[[月球背面|月球背面]]北半部一座古老的大[[撞击坑|撞击坑]]，约形成于39.2-38.5亿年前的[[酒海纪|酒海纪]]<ref name=LICD>Lunar Impact Crater Database</ref>，其名称取自二十世纪[[德国|德裔]][[美国|美籍]][[科普作家|科普作家]]"威利·莱伊"(Willy Ley，1906年－1969年)，1970年被[[国际天文学联合会|国际天文学联合会]]批准接受。

==描述==  
该陨坑西侧毗邻[[维纳环形山|维纳环形山]]、西北偏北靠近更大的环壁平原-[[坎贝尔环形山|坎贝尔环形山]]、[[朗之万环形山|朗之万环形山]]和[[戈洛文陨石坑|戈洛文陨石坑]]分别位于它的东北偏东和东南偏东，东南偏南及南面则坐落了[[阿普尔顿环形山|阿普尔顿环形山]]和[[冯·诺伊曼环形山|冯·诺伊曼环形山]]<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_32_lo.pdf Crater Ley on the map LAC-32.]</ref>。该陨坑中心[[月面坐标|月面坐标]]为{{Coord|42.11|N|154.84|E|globe:Moon|nogoogle=1|noosm=1}}，直径81.1公里<ref name="IAU">[http://planetarynames.wr.usgs.gov/Feature/3372 Directory of the International Astronomical Union]</ref>，深度约2.8公里<ref name="LICD"/>。

莱伊环形山呈多边形状，且已严重损毁。西南侧切入了[[冯·诺伊曼环形山|冯·诺伊曼环形山]]，而西北部则重叠在[[坎贝尔环形山|坎贝尔环形山]]的东南壁上。该陨坑坑壁已磨损并覆盖了众多的小陨坑，只有西侧部分保存最完整。环形山坑壁高出周边地形1360米<ref name="LICD"/>，内部容积约有6107公里<sup>3</sup><ref name="LICD"/>。坑底西南部因覆盖了[[冯·诺伊曼环形山|冯·诺伊曼环形山]]形成时溅射的岩石及它的外侧垒而崎岖不平，其余地表则相对较为平坦，密布有大量的小撞击坑，中心点西北坐落了一座醒目的杯状小陨坑。

==参考文献==
{{reflist}}

==另请参阅==
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | date = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | date = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | date = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| date = 1971| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | date = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co.
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | date = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | date = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | date = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revised
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | date = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | date = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==外部链接==
*[http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Ley%7C0 月球数码摄影图集]
*[http://the-moon.wikispaces.com/Ley 维基月球在线解说-冯·诺伊曼环形山]
*[http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月.]

[[Category:月球环形山|L]]