在[[數學|數學]]中，'''微分同胚'''是適用於[[微分流形|微分流形]][[範疇_(數學)|範疇]]的同構概念。這是從微分流形之間的可逆映射，使得此映射及其逆映射均為光滑（即無窮可微）的。

== 定義 ==
對給定的兩個微分流形<math>M, N</math>，若對光滑映射<math>f: M \to N</math>，存在光滑映射<math>g: N \to M</math>使得<math>f \circ g = \mathrm{id}_N</math>、<math>g \circ f = \mathrm{id}_M</math>，則稱<math>f</math>為微分同胚。此時逆映射<math>g</math>是唯一的。

若在微分流形<math>M, N</math>之間存在微分同胚，則稱<math>M</math>與<math>N</math>是微分同胚的，通常記為<math>M \simeq N</math>。

對於<math>C^r</math>流形，可採同樣辦法定義<math>C^r</math>微分同胚之概念。

== 例子 ==
考慮
: <math>\mathbb{R}/\mathbb{Z} \simeq S^1</math>
此微分同胚可由下述映射給出：
:<math>x\mapsto e^{2\pi ix}.</math>

== 與同胚的關係 ==
對維度<math>\leq 3</math>的流形，可證明[[同胚|同胚]]的流形必為微分同胚；換言之，此時流形上的'''拓撲結構確定了微分結構'''。在四維以上則存在反例，最早的構造是[[約翰·米爾諾|約翰·米爾諾]]的[[七維怪球|七維怪球]]，米爾諾更證明了七維球上恰有28種微分流形結構，它們都可表成某個在<math>S^4</math>上的<math>S^3</math>-叢。在1980年代，[[西蒙·唐納森|西蒙·唐納森]]與[[邁克爾·哈特利·弗里德曼|邁克爾·哈特利·弗里德曼]]的證明在<math>\mathbb{R}^4</math>上有不可數個相異的微分結構。

== 外部連結 ==
* {{springer|id=D/d031650|title=Diffeomorphism|author=D.V. Anosov}}

[[Category:微分幾何|W]]
[[Category:微分拓扑学|Category:微分拓扑学]]
[[Category:同胚|Category:同胚]]
[[Category:光滑函数|Category:光滑函数]]
[[Category:数学物理|Category:数学物理]]