{{lunar crater data|
 image = Wan-Hoo crater 5026 h3.jpg|
 caption=[[月球轨道器5号|月球轨道器5号]]的拍摄的斜视图|
 latitude=9.96|
 N_or_S=S|
 longitude=138.9|
 E_or_W=W|
 diameter=53.3公里|
 depth=2.4公里|
 colong=139|
 eponym=[[万户|万户]]}}
'''万户环形山'''（Wan-Hoo）是位于[[月球背面|月球背面]][[赤道|赤道]]区的一座大型[[撞击坑|撞击坑]]，约形成于[[酒海纪|酒海纪]]代，以十六世纪[[中国|中国]]官员[[万户|万户]]之名命名，1970年被[[国际天文联合会|国际天文联合会]]正式批准接受。

==描述==
该陨坑西北毗邻[[谢切诺夫环形山|谢切诺夫环形山]]、北面坐落着[[瓦维洛夫环形山|瓦维洛夫环形山]]、巨大的[[赫茨普龙环形山|赫茨普龙环壁平原]]位于它的东北、东侧则是[[埃文斯环形山|埃文斯环形山]]、南面横亘着[[帕申环形山|帕申环形山]]<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_88_wac.pdf Crater Wan-Hoo on the LAC-88 map]</ref>。该陨坑的中心[[月面坐标|月面坐标]]为{{Coord|9.96|S|138.91|W|globe:Moon|nogoogle=1|noosm=1}}，直径53.3公里<ref name=IAU>[http://planetarynames.wr.usgs.gov/Feature/6484 Directory of the International Astronomical Union]</ref>，深度2.4公里<ref name=LICD>Lunar Impact Crater Database</ref>。

该陨坑充塞着赫茨普龙环形山撞击时抛射的溅射物，外观已发生很大改变。环陨坑四壁覆盖着数座小陨坑，坑壁高出周边地形1140米，坑内容积约2200公里。碗形的坑底地势崎岖，地表散布着一些大小不同的陨石坑。大卫星坑埃文斯 Q连接在它的东南偏东，另一座年轻的小陨坑则附靠在它的东南侧壁上，同样，在它的西侧壁上也坐落着一座杯状的小陨坑。

该特征的名称来源[[万户|万户]]，为一些近代西方文章中杜撰的“中国传统中记录的首位太空人”。

==卫星陨石坑==
按惯例，最靠近万户环形山的卫星坑在月图上以字母标注在该坑中心点的旁边。
[[image:Wan-Hoo_satellite_craters_map.jpg|300px]]

{| class=wikitable
!width=25% style=background:#eeeeee; |万户 
!width=25% style=background:#eeeeee; |纬度
!width=25% style=background:#eeeeee; |经度
!width=25% style=background:#eeeeee; |直径
|-
|align=center|T
|align=center|10.0° S
|align=center|140.4° W
|align=center|21 公里
|}{{clear}}
==参考文献==
{{Reflist}}
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | year = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | year = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | year = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| year = 1971| pmid = | pmc =| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | year = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | year = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | year = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | year = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revision
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | year = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | year = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==另请参阅==
*[[万户|万户]]
*[[嫦娥一号|嫦娥一号]]

==外部链接==
* [http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Wan-Hoo%7C0 月球数码摄影图集]
* [http://the-moon.wikispaces.com/Wan-Hoo+%28Van-Gu%29 维基月球在线解说-万户环形山]
* [http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月.]

[[Category:月球环形山|W]]
[[Category:LQ16方格|W]]