...
2016年前[[Google翻译|Google翻译]]的大部分语言对采用的都是统计机器翻译的方法<ref>Google Translate FAQ http://www.google.com/intl/en/help/faq_translation.html </ref>。而Google亦在此本领域保持领先地位，在[[美国国家标准局|美国国家标准局]]组织的机器翻译评测中遥遥领先。<ref>NIST 2006 Machine Translation Evaluation Official Results http://www.nist.gov/speech/tests/mt/2006/doc/mt06eval_official_results.html</ref>但[[Google翻译|Google翻译]]在2016年11月开始使用[[神經機器翻譯|神經機器翻譯]]作为主要翻译系统，并开发了[[Google神經機器翻譯系統|Google神經機器翻譯系統]]。此外，常用的基于统计法机器翻译的系统还包括[[Bing翻译|Bing翻译]]<ref> Microsoft Translator Help http://www.microsofttranslator.com/help/?FORM=R5FD</ref>和百度翻译等。
...
于2012年提出的[[LEPOR|LEPOR]]评价指标<ref>Aaron L.-F. Han, Derek F. Wong and Lidia S. Chao 2012. LEPOR: A Robust Evaluation Metric for Machine Translation with Augmented Factors. Proc. of the 24th International Conference on Computational Linguistics (COLING 2012): Posters, pages 441–450, Mumbai, India</ref>在权威的计算语言学会统计机器翻译Workshop（ACL-WMT）的八个语言对语料上的实验得到了比BLEU与人的评价更高相关性的分数<ref>http://aclweb.org/anthology-new/C/C12/C12-2044.pdf</ref>。
...
统计机器翻译同大多数的机器学习方法相类似，有训练及解码两个阶段，其中训练阶段的目标是获得模型参数，而解码阶段的目标则是利用所估计的参数和给定的优化目标，获取待翻译语句的最佳翻译结果。对于基于短语的统计机器翻译来说，“训练”阶段较难界定，严格来说，只有最小错误率训练一个阶段可称为训练。但是一般来说，词对齐和短语抽取阶段也被归为训练阶段。<ref>Moses Training Tutorial http://www.statmt.org/moses/?n=FactoredTraining.HomePage</ref>
...
在短语抽取完毕后，可进行短语特征提取，即计算短语翻译概率及短语的词翻译概率。该需要对抽取的所有短语进行两次排序，一般来说，中等规模（百万句语料）的系统亦需要进行外部排序，磁盘读写速度对处理时间影响极大。建议在高速磁盘上运行。参考运行时间及磁盘空间消耗：前述千万句语料库，限制短语长度7，外部排序运行于SCSI Raid 0+1磁盘阵列，运行时间3日11小时，峰值磁盘空间消耗813GB。<ref>Q. Gao, 2008 Parallelizing the Training Procedure of Statistical Phrase-based Machine Translation, Student Research Symposium Carnegie Mellon University. http://www.lti.cs.cmu.edu/SRS/2008-abstracts/talks/GaoTalk.pdf{{dead link|date=2018年1月 |bot=InternetArchiveBot |fix-attempted=yes }}</ref>
...
* [http://www.statmt.org/ Statistical Machine Translation]—研究介绍、会议、语料及软件列表
* [http://code.google.com/p/giza-pp/ GIZA++:开源词对齐软件]
* [http://www.cs.cmu.edu/~qing MGIZA++/PGIZA++: GIZA++的并行化版本]
* [http://www.statmt.org/moses/ Moses:开源机器翻译软件]
* [http://www-nlp.stanford.edu/links/statnlp.html Annotated list of statistical natural language processing resources]
