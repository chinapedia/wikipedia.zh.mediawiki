{{lunar crater data|
 image= EijkmanCrater.jpg|240px
 caption= [[月球勘测轨道飞行器|月球勘测轨道飞行器]]拍摄的图像| 
 latitude=63.21|
 N_or_S=S|
 longitude=142.51|
 E_or_W=W|
 diameter=56.36公里|
 depth=2.43公里|
 colong=144|
 eponym=[[克里斯蒂安·艾克曼|克里斯蒂安·艾克曼]]}}
'''艾克曼环形山'''（Eijkman）是位于[[月球背面|月球背面]]南半部一座古老的大[[撞击坑|撞击坑]]，约形成于39.2-38.5亿年前的[[酒海纪|酒海纪]]<ref name=LICD>Lunar Impact Crater Database</ref>，其名称取自[[荷兰|荷兰]][[医生|医生]]、[[病理学|病理学家]][[克里斯蒂安·艾克曼|克里斯蒂安·艾克曼]]（1858年-1930年），1970年被[[国际天文学联合会|国际天文学联合会]]批准接受。

==描述==
该陨坑东南靠近[[勒梅特环形山|勒梅特环形山]]、西南偏南毗邻[[克罗姆林环形山|克罗姆林环形山]]，而巨大的[[斐索环形山|斐索环形山]]则坐落在它的东北方<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_133_lo.pdf Crater Eijkman on the map LAC-133.]</ref>。该陨坑中心[[月面坐标|月面坐标]]为{{Coord|63.21|S|142.51|W|globe:Moon|nogoogle=1|noosm=1}}，直径56.36公里<ref name="IAU">[http://planetarynames.wr.usgs.gov/Feature/1737 Directory of the International Astronomical Union]</ref>，深度约2.43公里<ref name="LICD"/>。

艾克曼环形山边缘轮廓清晰，未受到明显的撞击侵蚀，但环坑沿及北侧内壁嵌入有一些细小的坑穴。沿西侧内壁边沿显示分布了一些纤细的阶坡结构。它的坑壁最大高出周边地形1180米<ref name="LICD"/>，内部容积约2638立方千米<ref name="LICD"/>。坑底表面较为粗糙，散布有许多小撞击坑，中心点附近坐落了一座开口朝南的马蹄状中央峰。

==卫星陨石坑==
按惯例，最靠近艾克曼环形山的卫星坑将在月图上以字母标注在它的中心点旁边。
[[Image:EijkmanCraterSAT.jpg|300px]]拍摄的卫星坑图]]

{|cellpadding="3" border="1" cellspacing="0"
!width="25%" style="background:#eeeeee;" |艾克曼
!width="25%" style="background:#eeeeee;" |纬度
!width="25%" style="background:#eeeeee;" |经度 
!width="25%" style="background:#eeeeee;" |直径
|-
|align="center"|D
|align="center"|62.3° S
|align="center"|136.9° W
|align="center"|25 公里
|}{{clear}}

==参考资料==
{{Reflist}}

==另请参阅==
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | date = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | date = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | date = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| date = 1971| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | date = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co.
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | date = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | date = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | date = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revised
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | date = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | date = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==外部链接==
* [http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Eijkman%7C0 月球数码摄影图集]
* [http://the-moon.wikispaces.com/Eijkman 维基月球在线解说-艾克曼环形山]
* [http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月.]

[[Category:月球环形山|E]]