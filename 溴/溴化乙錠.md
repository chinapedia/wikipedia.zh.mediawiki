{{多個問題|
{{expert|time=2013-03-26T12:18:09+00:00}}
{{Expand English|Ethidium bromide}}
}}
{{chembox
 | verifiedrevid = 443740742
 | ImageFile = Ethidium bromide.svg
 <!-- also image EthidiumBromide.png exists -->
 | ImageSize = 
 | ImageFile1 = Ethidium-bromide-from-monohydrate-xtal-1971-3D-balls-B.png
 | ImageFile2 = Ethidium-bromide-monohydrate-xtal-1971-3D-SF.png
 | IUPACName = 3,8-Diamino-5-ethyl-6-phenylphenanthridinium bromide
 | OtherNames = 2,7-Diamino-10-ethyl-6-phenylphenanthridinium bromide, 2,7-Diamino-10-ethyl-9-phenylphenanthridinium bromide, 3,8-Diamino-1-ethyl-6-phenylphenantridinium bromide, 5-Ethyl-6-phenyl-phenanthridine-3,8-diamine bromide, Ethidium bromide, Homidium bromide, EtBr
 | Section1 = {{Chembox Identifiers
 |   ChemSpiderID_Ref = {{chemspidercite|correct|chemspider}}
 | ChemSpiderID = 14034
 | UNII_Ref = {{fdacite|correct|FDA}}
 | UNII = 059NUO2Z1L
 | InChI = 1/C21H19N3.BrH/c1-2-24-20-13-16(23)9-11-18(20)17-10-8-15(22)12-19(17)21(24)14-6-4-3-5-7-14;/h3-13,23H,2,22H2,1H3;1H
 | InChIKey = ZMMJGEGLRURXTF-UHFFFAOYAD
 | ChEMBL_Ref = {{ebicite|correct|EBI}}
 | ChEMBL = 284328
 | StdInChI_Ref = {{stdinchicite|correct|chemspider}}
 | StdInChI = 1S/C21H19N3.BrH/c1-2-24-20-13-16(23)9-11-18(20)17-10-8-15(22)12-19(17)21(24)14-6-4-3-5-7-14;/h3-13,23H,2,22H2,1H3;1H
 | StdInChIKey_Ref = {{stdinchicite|correct|chemspider}}
 | StdInChIKey = ZMMJGEGLRURXTF-UHFFFAOYSA-N
 | CASNo_Ref = {{cascite|correct|CAS}}
 | CASNo =  1239-45-8
 |   EINECS = 214-984-6
 |   PubChem = 14710
 |   ATCvet = yes
 |   ATCCode_prefix = P51
 |   ATCCode_suffix = AX06
 |   ChEBI_Ref = {{ebicite|correct|EBI}}
 | ChEBI = 4883
 | SMILES = CC[n+]1c2cc(ccc2c3ccc(cc3c1c4ccccc4)N)N.[Br-]
 |   RTECS = SF7950000
 |   KEGG_Ref = {{keggcite|correct|kegg}}
 | KEGG = C11161
   }}
 | Section2 = {{Chembox Properties
 |   Formula = C<sub>21</sub>H<sub>20</sub>BrN<sub>3</sub>
 |   MolarMass = 394.294 g/mol
 |   Appearance = 紫红色固体
 |   Density = 
 |   MeltingPt = 260 - 262 °C
 |   BoilingPt = 
 |   Solubility = ~ 40 g/l
   }}
 | Section3 = {{Chembox Hazards
 |   MainHazards = 
 |   FlashPt = > 100 °C
 |   Autoignition = 
 |   NFPA-H = 3
 |   NFPA-F = 1
 |   NFPA-R = 0
 |   NFPA-O = BIO
 |   RPhrases = {{R25}} {{R36/37/38}} {{R46}}
 |   SPhrases = {{S22}} {{S24/25}} {{S26}} {{S36/37/39}} {{S45}} {{S53}}
   }}
 }}
'''溴化乙錠'''（縮寫EtBr，EB）是一種核酸染料，常在[[瓊脂糖凝膠電泳|瓊脂糖凝膠電泳]]中用於[[核酸|核酸]][[染色|染色]]。在[[紫外光|紫外光]]的照射下，未與核酸結合的溴化乙錠可被激發出橙紅色螢光，在與[[DNA|DNA]]或雙股[[RNA|RNA]]結合時，螢光強度會增強20倍，使得核酸電泳後的膠片可以辨識核酸的相對位置。在核酸分子中，EB分子插入到兩層[[鹼基對|鹼基對]]之間。

==毒性==

溴化乙锭分子结构扁平，同时分子中还含有[[季铵阳离子|季铵阳离子]]结构，使得溴化乙锭能插入DNA[[碱基对|碱基对]]之间并使相邻碱基对间距离增大，从而引起DNA框架结构的变动，并致使三联密码转录和翻译时发生较多遗传信息的改变，最终诱发[[移码突变|移码突变]]。故溴化乙錠是一種強的[[誘變劑|誘變劑]]，可[[致癌|致癌]]或[[致畸|致畸]]。<ref>{{Cite book | author = 张景海 | title = 药学分子生物学（第八版）| location = 北京 | publisher = 人民卫生出版社 | date = 2016-2 | pages = 174-179 |  ISBN = 978-7-117-22373-7 | accessdate = 2017-9 | language = zh }}</ref>

大学生物实验经常会使用此染料，操作时必须要佩戴手套并且尽可能的避免任何接触。

[[File:Ethidium-bromide-abs.png|220px]]

==参考文献==
{{reflist}}

== 外部链接 ==
{{refbegin}}
* Borst P. Ethidium DNA agarose gel electrophoresis: how it started. IUBMB Life. 2005 Nov;57(11):745-7. PMID 16511967
* PMID 68004996, PubMed's MeSH term: Ethidium
{{refend}}

[[Category:菲啶染料|Category:菲啶染料]]
[[Category:季铵盐|Category:季铵盐]]
[[Category:突變原|Category:突變原]]