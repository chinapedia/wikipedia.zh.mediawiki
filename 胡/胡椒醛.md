{{chembox
| Verifiedfields = changed
| Watchedfields = changed
| verifiedrevid = 394986228
| Name = 胡椒醛
| ImageFile = Piperonal structure.png
| ImageName = 
| IUPACName = 1,3-Benzodioxole-5-carbaldehyde
| OtherNames= Heliotropin; Heliotropine; Piperonyl aldehyde; Protocatechuic aldehyde methylene ether; 3,4-methylenedioxybenzaldehyde;
|Section1={{Chembox Identifiers
| UNII_Ref = {{fdacite|correct|FDA}}
| UNII = KE109YAK00
| ChEBI_Ref = {{ebicite|changed|EBI}}
| ChEBI = 8240
| ChEMBL_Ref = {{ebicite|changed|EBI}}
| ChEMBL = 271663
| StdInChI_Ref = {{stdinchicite|changed|chemspider}}
| StdInChI = 1S/C8H6O3/c9-4-6-1-2-7-8(3-6)11-5-10-7/h1-4H,5H2
| StdInChIKey_Ref = {{stdinchicite|changed|chemspider}}
| StdInChIKey = SATCULPHIDQDRE-UHFFFAOYSA-N
| SMILES1 = c1cc2c(cc1C=O)OCO2
| CASNo_Ref = {{cascite|correct|CAS}}
| CASNo = 120-57-0
| ChemSpiderID_Ref = {{chemspidercite|changed|chemspider}}
| ChemSpiderID = 13859497
| SMILES = O=Cc1ccc2OCOc2c1
  }}
|Section2={{Chembox Properties
| C=8 | H=6 | O=3
| Appearance = 透眀結晶體<ref name=Merck>{{Merck12th}}</ref>
| Density = 1.337 g/cm<sup>3</sup>
| MeltingPtC = 37
| MeltingPt_ref = <ref name=Merck/>
| BoilingPtC = 263
| BoilingPt_ref = <ref name=Merck/>
| Solubility = Soluble in 500 parts<ref name=Merck/>
  }}
|Section3={{Chembox Hazards
| MainHazards =
| FlashPt =
| AutoignitionPt =
| LD50 = 2700 mg/kg (orally in rats)<ref name=Merck/> }}
}}

'''胡椒醛'''是一種常用於香水及食品中香料的[[有机化合物|有机化合物]]化合物添加劑<ref name=Ullmann>Karl-Georg Fahlbusch, Franz-Josef Hammerschmidt, Johannes Panten, Wilhelm Pickenhagen, Dietmar Schatkowski, Kurt Bauer, Dorothea Garbe and Horst Surburg "Flavors and Fragrances" in Ullmann's Encyclopedia of Industrial Chemistry, Wiley-VCH, Weinheim, 2003. {{DOI|10.1002/14356007.a11_141}}</ref> ，分子結構上與其他[[芳香族|芳香族]]，[[醛|醛]]化合物如[[苯甲醛|苯甲醛]]和[[香蘭素|香蘭素]]相似。

==天然存在==
胡椒醛天然存在于各种植物。例子包括[[莳萝|莳萝]]，[[香莢蘭|香草]]，[[香堇菜|香堇菜]]，和[[黑胡椒|黑胡椒]]。

==制備==
胡椒醛可通過[[氧化還原反應|氧化]][[異黃樟腦|異黃樟腦]]來取得，或以[[兒茶酚|兒茶酚]]或[[1,3-苯並|1,2-亞甲二氧基苯]]為原料，逐步合成至胡椒醛。

==参考资料==
{{reflist}}

[[Category:苯并二茂|Category:苯并二茂]]