{{lunar crater data|
 image=Yakovkin crater 4186 h1.jpg|240px|
 caption=[[月球轨道器4号|月球轨道器4号]]拍摄的图像|
 latitude=54.42|
 N_or_S=S|
 longitude=78.93|
 E_or_W=W|
 diameter=35.92公里|
 depth=2.12公里|
 colong=80|
 eponym=''[[阿韦尼尔·亚历山德罗维奇·雅科夫金|阿·亚·雅科夫金]]''}}

'''雅科夫金陨石坑'''（Yakovkin）是位于[[月球正面|月球正面]]西南边沿上的一座[[撞击坑|撞击坑]]，约形成于39.2-38.5亿年前的[[酒海纪|酒海纪]]<ref name=LICD>Lunar Impact Crater Database</ref>，其名称取自[[前苏联|前苏联]][[天文学家|天文学家]][[阿韦尼尔·亚历山德罗维奇·雅科夫金|阿韦尼尔·亚历山德罗维奇·雅科夫金]]（1887年-1974年），1985年被[[国际天文学联合会|国际天文学联合会]]批准接受。

==描述==  
该陨坑位于形成于[[酒海纪|酒海纪]]期，直径630公里的[[孟德尔-里德伯盆地|孟德尔-里德伯盆地]]内，东南靠近[[潘格雷环形山|潘格雷环形山]]，在被[[国际天文学联合会|国际天文学联合会]]更名前，曾称之为卫星坑“[[潘格雷环形山|潘格雷 H]]”<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_124_wac.pdf Crater Yakovkin on the map LAC-124.]</ref>。陨坑中心[[月面坐标|月面坐标]]为{{Coord|54.42|S|78.93|W|globe:Moon|nogoogle=1|noosm=1}}，直径35.92公里<ref name="IAU">[http://planetarynames.wr.usgs.gov/Feature/6626 Directory of the International Astronomical Union]</ref>，深度约2.12公里<ref name="LICD"/>。

雅科夫金陨石坑凹凸不平的坑壁轮廓形似多边形状，坑内已被[[熔岩|熔岩]]淹没，形成一圈相对较小的内侧壁，坑壁最大高出周边地形990米<ref name="LICD"/>，内部容积约932公里<sup>3</sup><ref name="LICD"/>。坑内除散布有一些细小的坑穴外，无其它地貌特征。环陨坑周边的地形已发生改变，显示分布有一些小或浅的撞击坑，而西北更远处的地形则更为崎岖粗糙。

该陨坑位置非常接近在[[地球|地球]]上不利于观察的[[月球|月球]]边沿处，且外观因[[透视|透视]]而变形。

==参考文献==
{{reflist}}

==另请参阅==
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | date = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | date = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | date = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| date = 1971| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | date = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co.
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | date = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | date = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | date = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revised
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | date = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | date = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==外部链接==
* [http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Yakovkin%7C0 月球数码摄影图集]
* [http://the-moon.wikispaces.com/Yakovkin 维基月球在线解说-雅科夫金陨石坑]
* [http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月.]

[[Category:月球环形山|Y]]