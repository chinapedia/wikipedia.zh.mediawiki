{{lunar crater data|
 image=Horrebow crater 4158 h3.jpg|240px|
 caption=月球轨道器4号拍摄的图像|
 latitude=58.80|
 N_or_S=N|
 longitude=40.93|
 E_or_W=W|
 diameter=24.98公里|
 depth=1.90公里|
 colong=41|
 eponym=佩德·赫瑞鲍}}
'''赫瑞鲍陨石坑'''（Horrebow）是[[月球正面|月球正面]]位于[[冷海|冷海]]西北边沿的一座小[[撞击坑|撞击坑]]，其名称取自[[丹麦|丹麦]][[天文学家|天文学家]]"佩德·赫瑞鲍"(Peder Horrebow，1679年-1764年)，1935年被[[国际天文学联合会|国际天文学联合会]]正式批准接受。

==描述==  
该陨坑的北部重叠在大小类似的卫星坑"赫瑞鲍 A"西南部上，北邻[[约·赫歇尔环形山|约·赫歇尔环形山]]，东近[[罗宾逊陨石坑|罗宾逊陨石坑]]<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_11_lo.pdf Crater Horrebow on the map LAC-11.]</ref>。其中心[[月面坐标|月面坐标]]为{{Coord|58.80|N|40.93|W|globe:Moon|nogoogle=1|noosm=1}}，直径24.98公里<ref name="IAU">[http://planetarynames.wr.usgs.gov/Feature/2557 Directory of the International Astronomical Union]</ref>，深度约1.90公里<ref name=LICD>Lunar Impact Crater Database</ref>。

赫瑞鲍陨石坑外观轮廓大体圆状，坑壁边沿峻峭、清晰，没有受到明显的侵蚀磨损，陨坑东侧内壁较其它部分更狭窄陡峭，内侧壁显示已[[山崩|崩塌]]或滑坡，环坡底分布着一圈[[岩屑堆|岩屑堆]]。赫瑞鲍陨石坑坑壁最大高出周边地形850米<ref name="LICD"/>，内部容积约403公里<sup>3</sup><ref name="LICD"/>。由于东西壁不对称，使圆形的坑底显得略微偏东，坑内地形较为崎岖，没有醒目的地貌结构。

==卫星陨石坑==
按惯例，最靠近赫瑞鲍陨石坑的卫星坑在月图上以字母标注在该坑中心点的旁边。
[[image:HorrebowCraterSAT.jpg|350px]]拍摄]]

{| class="wikitable"
!width="25%" style="background:#eeeeee;" |赫瑞鲍 
!width="25%" style="background:#eeeeee;" |纬度
!width="25%" style="background:#eeeeee;" |经度
!width="25%" style="background:#eeeeee;" |直径
|-
|align="center"|A
|align="center"|59.2° N
|align="center"|40.4° W
|align="center"|25 公里
|-
|align="center"|B
|align="center"|58.7° N
|align="center"|42.7° W
|align="center"|13 公里
|-
|align="center"|C
|align="center"|56.9° N
|align="center"|36.0° W
|align="center"|5 公里
|-
|align="center"|D
|align="center"|57.9° N
|align="center"|38.7° W
|align="center"|5 公里
|-
|align="center"|G
|align="center"|59.7° N
|align="center"|41.7° W
|align="center"|8 公里
|}{{clear}}
==参考文献==
{{reflist}}

==另请参阅==
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | date = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | date = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | date = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| date = 1971| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | date = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co.
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | date = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | date = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | date = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revised
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | date = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | date = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==外部链接==
*[http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Horrebow%7C0 月球数码摄影图集]
*[http://the-moon.wikispaces.com/Horrebow 维基月球在线解说-赫瑞鲍陨石坑.]
* [http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月.]

[[Category:月球环形山|H]]