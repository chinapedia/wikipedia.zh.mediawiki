...
'''-{zh-hans:队列;zh-hant:佇列}-'''，又稱為'''-{zh-hans:伫列;zh-hant:隊列}-'''（queue），是[[先进先出|先进先出]]（FIFO, First-In-First-Out）的[[线性表|线性表]]。在具体应用中通常用[[链表|链表]]或者[[数组|数组]]来实现。队列只允许在后端（称为''rear''）进行插入操作，在前端（称为''front''）进行删除操作。
