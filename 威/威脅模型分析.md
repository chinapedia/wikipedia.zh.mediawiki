{{no footnotes|time=2014-02-14T11:03:09+00:00}}
'''威脅模型分析''' （threat modeling）是尋找系統潛在威脅以建立對抗的策略，以建立安全的系統。它屬於[[資訊安全|資訊安全]]的議題，並將問題劃分成「尋找針對特定技術的威脅模型」與「基於威脅模型建置更安全的系統」。
...
:這類分析方式又稱系統導向（system-centric）、設計導向（design-centric）或架構導向（architecture-centric）。思考點是系統的設計方式與系統的運作目的，並尋找對此系統或其內部模組相關類型的攻擊方式。[[微軟|微軟]]的SDL（Security Development Lifecycle）便是利用此方式。
...
==參考文獻==
Dan Griffin's Blog. "MS08-067 and the ripple effect of Windows security bugs". Retrieved from: http://www.jwsecure.com/2008/10/24/ms08-067-and-the-ripple-effect-of-windows-security-bugs/{{Reflist}}
...
* [http://www.ncassr.org/publications/index.php?id=277 威脅模型分析是基礎 - NCASSR Publications]{{dead link|date=2017年12月 |bot=InternetArchiveBot |fix-attempted=yes }}
