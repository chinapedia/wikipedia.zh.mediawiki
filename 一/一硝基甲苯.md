{{Chembox
| Verifiedfields = changed
| verifiedrevid = 411133631
|  Name = 2-Nitrotoluene
| ImageFile = O-Nitrotoluol.svg
| ImageSize = 100
| IUPACName = 1-methyl-2-nitro-benzene
| OtherNames = ''o''-Nitrotoluene
| Section1 = {{Chembox Identifiers
|  ChEMBL_Ref = {{ebicite|correct|EBI}}
| ChEMBL = 47047
| StdInChI_Ref = {{stdinchicite|correct|chemspider}}
| StdInChI = 1S/C7H7NO2/c1-6-4-2-3-5-7(6)8(9)10/h2-5H,1H3
| InChIKey = PLAZTCDQAHEYBI-UHFFFAOYSA-N
| InChI = 1S/C7H7NO2/c1-6-4-2-3-5-7(6)8(9)10/h2-5H,1H3
| CASNo_Ref = {{cascite|correct|??}}
| CASNo =  88-72-2
|  PubChem = 
|  ChemSpiderID_Ref = {{chemspidercite|correct|chemspider}}
| ChemSpiderID = 21106144
|  ChEBI_Ref = {{ebicite|changed|EBI}}
| ChEBI = 33098
| StdInChIKey_Ref = {{stdinchicite|changed|chemspider}}
| StdInChIKey = PLAZTCDQAHEYBI-UHFFFAOYSA-N
| SMILES = Cc1ccccc1[N+](=O)[O-]
|  StdInChI1 =1S/C7H7NO2/c1-6-4-2-3-5-7(6)8(9)10/h2-5H,1H3
  }}
| Section2 = {{Chembox Properties
|  Formula = C<sub>7</sub>H<sub>7</sub>NO<sub>2</sub>
|  MolarMass = 137.136
|  Appearance = 
|  Density = 
|  MeltingPt = -9.3°C
|  BoilingPt = 
|  Solubility =
  }}
| Section3 = {{Chembox Hazards
|  MainHazards = 
|  FlashPt = 
|  Autoignition =
  }}
}}

{{Chembox
|  Name = 3-Nitrotoluene
| ImageFile = M-Nitrotoluol.svg
| ImageSize = 100
| IUPACName = 1-methyl-3-nitro-benzene
| OtherNames = ''m''-Nitrotoluene
| Section1 = {{Chembox Identifiers
|  CASNo_Ref = {{cascite|changed|??}}
|  CASNo = 99-08-1
|  PubChem = 
|  ChemSpiderID = 21106146
|  SMILES = O=[N+]([O-])c1cccc(C)c1
|  InChI = InChI=1S/C7H7NO2/c1-6-3-2-4-7(5-6)8(9)10/h2-5H,1H3
  }}
| Section2 = {{Chembox Properties
|  Formula = C<sub>7</sub>H<sub>7</sub>NO<sub>2</sub>
|  MolarMass = 137.136
|  Appearance = 
|  Density = 
|  MeltingPt = 16.1°C
|  BoilingPt = 
|  Solubility =
  }}
| Section3 = {{Chembox Hazards
|  MainHazards = 
|  FlashPt = 
|  Autoignition =
  }}
}}

{{Chembox
|  Name = 4-Nitrotoluene
| ImageFile = P-Nitrotoluol.svg
| ImageSize = 50
| IUPACName = 1-methyl-4-nitrobenzene
| OtherNames = ''p''-Nitrotoluene
| Section1 = {{Chembox Identifiers
|  CASNo_Ref = {{cascite|changed|??}}
|  CASNo = 99-99-0
|  PubChem = 
|  ChemSpiderID = 13863774
|  SMILES = O=[N+]([O-])c1ccc(C)cc1
|  InChI = InChI=1S/C7H7NO2/c1-6-2-4-7(5-3-6)8(9)10/h2-5H,1H3
  }}
| Section2 = {{Chembox Properties
|  Formula = C<sub>7</sub>H<sub>7</sub>NO<sub>2</sub>
|  MolarMass = 137.136
|  Appearance = 
|  Density = 
|  MeltingPt = 51.7°C
|  BoilingPt = 
|  Solubility =
  }}
| Section3 = {{Chembox Hazards
|  MainHazards = 
|  FlashPt = 
|  Autoignition =
  }}
}}

'''一硝基甲苯'''（英语：'''Mononitrotoluene'''， '''methylnitrobenzene''' 或 '''nitrotoluene''' ('''MNT''' 或 '''NT''')）, 是三种[[化合物|化合物]]所形成的一类[[有机物|有机物]]，是一种[[甲苯|甲苯]]的[[硝化|硝化]]物，或者可以说是[[硝基苯|硝基苯]]的[[甲基化|甲基化]]物。它的[[化学式|化学式]]是 C<sub>6</sub>H<sub>4</sub>(CH<sub>3</sub>)(NO<sub>2</sub>).

[[甲基|甲基]]和[[硝基|硝基]]的相对位置可以不同，因此一硝基甲苯有三种[[同分异构体|同分异构体]]。

* '''''邻''-一硝基甲苯''' ('''ONT''')，又称为'''''o''-硝基甲苯''',或 '''2-硝基甲苯'''。是一种淡黄色液体，有[[苦杏仁|苦杏仁]]气味，没有[[吸湿性|吸湿性]]和[[腐蚀性|腐蚀性]]。

* '''''间''-一硝基甲苯''' ('''MNT''')，又称为 '''''m''-硝基甲苯''', 或 '''3-硝基甲苯'''。是一种黄绿色的液体，具有微弱的香味。

* '''''对''-一硝基甲苯''' ('''PNT''')，又称为'''''p''-硝基甲苯''', 或 '''4-硝基甲苯'''。是一种浅黄色物质，能形成[[菱形|菱形]]的晶体，并有几分令人愉快的，特有的[[苦杏仁|苦杏仁]]气味。它几乎不溶于水。

一硝基甲苯是[[颜料|颜料]]，[[抗氧化剂|抗氧化剂]]，农业化学品，感光化学品生产的重要原料。

''邻''-一硝基甲苯和''对''-一硝基甲苯也能被用作爆炸品检测的[[示踪剂|示踪剂]]。

== 参见 ==
*[[甲苯|甲苯]]
*[[二硝基甲苯|二硝基甲苯]](DNT)
*[[三硝基甲苯|三硝基甲苯]](TNT)
*[[硝基苯|硝基苯]]

{{Authority control}}
[[Category:爆炸品|Category:爆炸品]]
[[Category:硝基甲苯|Category:硝基甲苯]]