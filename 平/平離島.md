
{{Infobox islands
| name             = 平離島<!-- mandatory -->
| native name      = <!-- or |local name= -->平離
| native name link = 
| native_name_lang =
| sobriquet        = <!-- or |nickname= -->
| image name       = [[File:Hirapanari-Jima_Island.jpg|300px]]
| image size       = 
| image caption    = 從石垣島川平石崎地區眺望平離島
| image alt        = 
| map              = 
| map_alt          = 
| map_width        = 
| map_caption      = 
| map_relief       = 
| label            = 
| label_position   = 
| lat_d            =24
| lat_m            =29
| lat_s            =4
| lat_NS           = N
| long_d           =124 
| long_m           =6
| long_s           =58
| long_EW          = E
| x                = 
| y                = 
| x%               = 
| y%               = 
| etymology        = 
| location         = <!-- mandatory, body of water where located -->[[東海|東海]]
| type             = 
| pop              = 
| region           = 
| dim              = 
| scale            = 
| source           = 
| display          = 
| format           = 
| GridReference    = <!-- UK only -->
| archipelago      = [[八重山群島|八重山群島]]
| waterbody        = 
| total islands    = 
| major islands    = 
| area sqmi        = <!-- or |area sqft= or |area acre= -->
| area_km2 =0.02
| area footnotes   = 
| rank             = 
| length mi        = <!-- or |length ft= -->
| length footnotes = 
| width mi         = <!-- or |width ft= -->
| width footnotes  = 
| coastline mi     = <!-- or |coastline ft= -->
| coastline footnotes = 
| elevation ft     = 
| elevation_m =34
| elevation footnotes = 
| highest mount    = 
| Country heading  = 
| country          = <!-- mandatory -->{{JPN}}
| country admin divisions title      = 地域
| country admin divisions            = [[沖繩縣|沖繩縣]][[石垣市|石垣市]]
| country admin divisions title 1    = 
| country admin divisions 1          = 
| country admin divisions title 2    = 
| country admin divisions 2          = 
| country capital type               = 
| country capital                    = 
| country largest city type          = 
| country largest city               = 
| country capital and largest city   = 
| country largest city population    = 
| country leader title               = 
| country leader name                = 
| country area sqmi                  = <!-- or |country area sqft= or |country area acre= -->
| country 1                          = 
| country 1 admin divisions title    = 
| country 1 admin divisions          = 
| country 1 admin divisions title 1  = 
| country 1 admin divisions 1        = 
| country 1 capital type             = 
| country 1 capital                  = 
| country 1 largest city type        = 
| country 1 largest city             = 
| country 1 capital and largest city = 
| country 1 largest city population  = 
| country 1 leader title             = 
| country 1 leader name              = 
| country 1 area sqmi                = <!-- or |country 1 area sqft= or |country 1 area acre= -->
| demonym          =
| population       = 0
| population as of = 
| population footnotes = 
| population rank =
| population rank max =
| density sqmi     = 
| density rank     =
| density footnotes = 
| ethnic groups    = 
| timezone1        = [[日本標準時間|日本標準時間]]
| utc_offset1      = +9
| timezone1_DST    = 
| utc_offset1_DST  = 
| website          = 
| additional info  = 
}}

'''平離島'''（{{jpn|j=平離島|hg=ひらばなりじま|rm=Hirabanari Jima|f=h}}）是[[八重山諸島|八重山諸島]][[石垣島|石垣島]]川平石崎北方的[[荒島|無人島]]，全島屬於[[沖繩縣|沖繩縣]][[石垣市|石垣市]]，也可以稱呼為平離。

地點位在[[石垣島|石垣島]]西北邊，川平灣西方突出的川平石崎北側約100公尺的位置，面積約0.02平方公尺，周圍被[[珊瑚礁|珊瑚礁]]包圍，退潮的時候可以從海岸線步行抵達。

過去抵達平離島的交通不方便，由於1999年9月渡假村集團[[地中海俱乐部|地中海俱樂部]]在川平石崎開幕，附近的道路陸續建造完成，近年來已經很容易抵達平離島。

2007年8月1日由於[[西表石垣國立公園|西表石垣島國立公園]]的擴張，平離島被指定為普通地域<ref>[http://www.env.go.jp/press/file_view.php?serial=9387&hou_id=8211 西表国立公園の公園区域及び公園計画の変更について] - 環境省</ref>。

== 參考資料 ==
<div class="references-small">
<references /></div>

== 相關條目 ==
* [[石垣市|石垣市]]
* [[日本地理|日本地理]]
* [[日本島嶼列表|日本島嶼列表]]

{{八重山群島}}
[[Category:八重山群島|Category:八重山群島]]