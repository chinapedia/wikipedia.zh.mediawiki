{{微積分學}}
'''线性微分方程'''是数学中常见的一类微分方程。指以下形式的[[微分方程|微分方程]]：

: <math> \mathcal{L}(y) = f \qquad \ldots \; \; (*)</math>

其中方程左侧的[[微分算子|微分算子]]<math>\mathcal{L}</math>是[[线性算子|线性算子]]，{{mvar|y}}是要解的未知函数，方程的右侧是一个已知函数。如果{{mvar|f}}({{mvar|x}}) {{math|{{=}}}} 0，那么方程(*)的解的线性组合仍然是解，所有的解构成一个[[向量空间|向量空间]]，称为'''解空间'''。这样的方程称为'''齐次线性微分方程'''。当{{mvar|f}}不是零函数时，所有的解构成一个[[仿射空间|仿射空间]]，由对应的齐次方程的解空间加上一个特解得到。这样的方程称为'''非齐次线性微分方程'''。线性微分方程可以是[[常微分方程|常微分方程]]，也可以是[[偏微分方程|偏微分方程]]。
...
一种解线性微分方程的方法是[[欧拉|欧拉]]发现的，他意识到这类方程的解都具有<math>e^{z x}</math>的形式，其中<math>z</math>是某个复数。因此，对于以下方程：
...
换成''z''<sup>''k''</sup>，便可得到特征方程。这个方程有''n''个解：''z''<sub>1</sub>, ..., ''z''<sub>''n''</sub>。把任何一个解代入''e''<sup> ''zx''</sup>，便可以得到微分方程的一个解：''e''<sup> ''z''<sub>''i''</sub>''x''</sup>。由于齐次线性微分方程满足[[叠加原理|叠加原理]]，因此这些函数的任意[[线性组合|线性组合]]仍然满足微分方程。

如果特征方程的根都不重复，我们便得到了微分方程的''n''个解。可以证明，这些解是[[线性独立|线性独立]]的。于是，微分方程的通解就是''y'' = ''C''<sub>1</sub>''e''<sup> ''z''<sub>''1''</sub>''x''</sup> + ''C''<sub>2</sub>''e''<sup> ''z''<sub>''2''</sub>''x''</sup> + …… + ''C''<sub>n</sub>''e''<sup> ''z''<sub>''n''</sub>''x''</sup>，其中''C''<sub>1</sub>、''C''<sub>2</sub>、……、''C''<sub>n</sub>是常数。
...
一般地，如果微分方程的系数''A<sub>i</sub>''都是实数，那么它的解也应该表示成实数的形式。假如特征方程有复数根，那么它一定是成对的，也就是说，如果''a'' + ''bi''是特征方程的根，那么''a'' - ''bi''也是一个根。于是，''y'' = ''e''<sup> (''a'' + ''bi'')''x''</sup>和''y'' = ''e''<sup> (''a'' - ''bi'')''x''</sup>都是微分方程的解。但这两个解都是复数的形式。考虑到这两个解的任意线性组合也仍然是微分方程的解，我们可以把这两个解相加，再[[除以二|除以2]]，利用[[欧拉公式|欧拉公式]]，便得到一个实数形式的解：''y'' = ''e''<sup> ''ax''</sup>cos''bx''。如果把两个解相减，再除以2i，便得到另一个实数形式的解：''y'' = ''e''<sup> ''ax''</sup>sin''bx''。于是，''y'' = ''C''<sub>1</sub>''e''<sup> ''ax''</sup>cos''bx'' + ''C''<sub>2</sub>''e''<sup> ''ax''</sup>sin''bx''就是微分方程的通解。
...
欲得到非齐次线性微分方程的通解，我们首先求出对应的齐次方程的通解，然后用[[待定系数法|待定系数法]]或{{Tsl|ja|定数変化法|常数变易法}}求出非齐次方程本身的一个特解，把它们相加，就是非齐次方程的通解。
...
这个方程可以用[[积分因子|积分因子]]求解，方法是把两边乘以<math>e^{\int f(x)\,dx}</math>：
