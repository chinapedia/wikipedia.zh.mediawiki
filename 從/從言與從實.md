'''從言'''（de dicto）與'''從實'''（de re）是對於[[信念|信念]]、想望、[[模態|模態]]等[[內涵|內涵性]]語句的兩種不同解讀方式。

'''從言'''是把詞語解讀為詞語反映的概念<ref>http://www.oxforddictionaries.com/definition/english/de-dicto</ref>，'''從實'''則是把詞語解讀為詞語指涉的實體對象<ref>http://www.oxforddictionaries.com/definition/english/de-re</ref>。
...
* 從言解讀：<math>\Box \exists x ( Ax )</math> （小明相信：存在x，x這次考試考了滿分）
* 從實解讀：<math>\exists x ( \Box Ax )</math> （存在x，小明相信x這次考試考了滿分）
...
本句有從言從實兩種解讀，翻譯成形式語言如下：
* 從言解讀：<math>\Box \exists ! x ( Rx \land Mx )</math> （小美想要：存在唯一的x，x是全國最有錢的人，且嫁給x）
* 從實解讀：<math>\exists ! x ( Rx \land \Box Mx )</math> （存在唯一的x，x是全國最有錢的人，且小美想要嫁給x）
...
本句有從言從實兩種解讀，翻譯成形式語言如下：
* 從言解讀：<math>\Box \forall x ( Mx \to Sx )</math> （必然地：若x是媽媽，則x是有孩子的）
* 從實解讀：<math>\forall x ( Mx \to \Box Sx )</math> （若x是媽媽，則x是必然有孩子的）
...
== 外部連結 ==
* {{en}} http://plato.stanford.edu/entries/prop-attitude-reports/dere.html
