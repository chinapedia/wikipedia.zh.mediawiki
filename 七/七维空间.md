在[[數學|數學]]中， 一個''n''[[實數|實數]]的序列可以被理解為''[[n維|n維]]''空間中的一個[[點|位置]]。當''n''等於七時， 所有這樣的位置的集合被稱為 '''七維空間'''。 通常這種空間被研究為一個[[向量空間|向量空間]]，而沒有任何距離的概念。 七維[[歐幾里得空間|歐幾里得空間]]是一個配備了一個[[歐幾里得距離|歐幾里得距離]]的七維空間，它由[[點積|點積]]定義。
