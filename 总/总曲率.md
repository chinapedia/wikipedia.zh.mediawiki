...
在[[数学|数学]]中的[[曲线微分几何|曲线微分几何]]的研究中, 一个[[浸入|浸入]]在平面上的曲线的'''总曲率'''是[[曲率|曲率]]的曲线积分:

:<math>\int_a^b k(s)\,ds.</math>

闭曲线的总曲率是 2π 的整数倍, 该整数称为曲线的'''指数'''或'''[[转数|转数]]'''. 其中[[转数|转数]]是单位切向量关于起点的[[绕数|绕数]], 或者等价的[[高斯映射|高斯映射]]的次数.
局部不变量曲率和整体拓扑不变量指数的关系是高维[[黎曼几何|黎曼几何]]的代表性结果，如[[高斯-博内定理|高斯-博内定理]] 。

== 不变量 ==
根据[[惠特尼-格劳斯坦定理|惠特尼-格劳斯坦定理]], 总曲率在曲线的[[正则同伦|正则同伦]]下不变: 总曲率是[[高斯映射|高斯映射]]的次数. 然而, 它不是同伦下的不变量：经历一个扭结将会更改转折点的数目.

相反, 关于曲线外一点的[[绕数|绕数]]在同伦下不变. 对于曲线上的点[[绕数|绕数]]将改变1.
