...
{{Link style|time=2015-12-11T07:33:54+00:00}}
當學習器去預測其未遇到過的輸入的結果時，會做一些假設（Mitchell, 1980）。而學習[[演算法|演算法]]中的'''歸納偏置'''（{{lang|en|Inductive bias}}）則是這些假設的集合。
...
== 歸納偏置的種類 ==
以下是機器學習中常見的歸納偏置列表：
* '''最大[[條件獨立性|條件獨立性]]'''（conditional independence）：如果假說能轉成[[贝叶斯概率|貝葉斯模型]]架構，則試著使用最大化條件獨立性。這是用於[[朴素貝葉斯分類器|朴素貝葉斯分類器]]（Naive Bayes classifier）的偏置。
...
== 另見 ==
* [[Bias|Bias]]
* [[認知偏誤|認知偏誤]] [[:en:Cognitive_bias|:en:Cognitive bias]]
