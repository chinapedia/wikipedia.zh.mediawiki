{{noteTA
|G1 = IT
}}

'''资源描述框架'''（{{lang-en|'''R'''esource '''D'''escription '''F'''ramework}}，縮寫：{{lang|en|'''RDF'''}}），是[[万维网联盟|万维网联盟]]（[[W3C|W3C]]）提出的一组[[标记语言|标记语言]]的{{tsl|en|Specification (technical standard)|技术规范}}，基於XML語法及XML Schema的資料類型以便更为丰富地描述和表达{{tsl|en|Web resource|网络资源}}的内容与结构。

== 参考文献 ==
{{Reflist}}
{{refbegin}}
* [http://zh.transwiki.org/cn/rdfprimer.htm W3C RDF Primer中文版]
{{refend}}

== 延伸閱讀 ==
* [http://www.w3.org/RDF/ W3C's RDF at W3C]: specifications, guides, and resources
* [http://www.w3.org/TR/2004/REC-rdf-mt-20040210/ RDF Semantics]: specification of semantics, and complete systems of inference rules for both RDF and RDFS

== 外部連結==
{{Commons category|Resource Description Framework}}
* {{DMOZ|Reference/Libraries/Library_and_Information_Science/Technical_Services/Cataloguing/Metadata/RDF/}}

== 参见 ==
{{Portal|互联网}}
* [[元数据|元数据]]
*[[语义网|语义网]]
*[[简单知识组织系统|简单知识组织系统]]
*[[Web_3.0|Web 3.0]]
*{{tsl|en|RDF Schema|RDF模式}}
*[[本体语言|本体语言]]
**[[网络本体语言|OWL]]

{{-}}
{{W3C规范和标准}}
{{语义网}}
{{Data Exchange}}

{{Authority control}}
[[Category:知识表示|Category:知识表示]]
[[Category:XML|Category:XML]]
[[Category:W3C标准|Category:W3C标准]]
[[Category:資源描述架構|]]

{{Compu-stub}}