{{lunar crater data|
 image=Il'in crater WAC.jpg|
 caption=[[月球勘测轨道飞行器|月球勘测轨道飞行器]]拍摄的图像|
 latitude=17.8|
 N_or_S=S|
 longitude=97.68|
 E_or_W=W|
 diameter=12.42公里|
 depth=2.02公里|
 colong=98|
 eponym=[[尼古拉·雅科夫列维奇·伊林|尼古拉·雅科夫列维奇·伊林]]}}
'''伊林陨石坑'''（Il'in）是位于[[月球背面|月球背面]][[东方海|东方海]]西部的一座[[撞击坑|撞击坑]]，其名称取自[[前苏联|前苏联]][[火箭|火箭]]技术先驱之一，气体动力研究所主管[[尼古拉·雅科夫列维奇·伊林|尼古拉·雅科夫列维奇·伊林]]（1901年-1937年），1985年被[[国际天文学联合会|国际天文学联合会]]正式接受。

==描述==
[[Image:Normal_ilin-clem1.jpg|240px]]拍摄的图像]] 
该陨坑西北靠近[[罗威尔撞击坑_(月球)|罗威尔环形山]]、东北毗邻[[蒙德环形山|蒙德环形山]]，较小的[[霍曼陨石坑|霍曼陨石坑]]位于它的东面，西南坐落醒目的[[弗里克塞尔陨石坑|弗里克塞尔陨石坑]]，而它的西面则绵延着环[[东方海|东方海]]的[[鲁克山脉|鲁克山脉]]<ref>[http://planetarynames.wr.usgs.gov/images/Lunar/lac_108_lo.pdf Crater Il'in on the map LAC-108.]</ref>。陨坑中心[[月面坐标|月面坐标]]为{{Coord|17.8|S|97.68|W|globe:Moon|nogoogle=1|noosm=1}}，直径12.42公里<ref>[http://planetarynames.wr.usgs.gov/Feature/2673 Directory of the International Astronomical Union]</ref>，深约2.02公里<ref name=LICD>Lunar Impact Crater Database</ref>。

伊林陨石坑外观呈圆碗状，带有一个平坦的小坑底，坑壁几乎未受到较大的撞击磨损。该陨坑的[[反照率|反照率]]明显高于周边[[月海|月海]]表面，呈现出年轻撞击坑的典型特征。它的坑壁最大高出周边地形460米<ref name="LICD"/>，内部容积约为76.63立方千米<ref name="LICD"/>。

在1985年被[[国际天文学联合会|国际天文学联合会]]正式命名前，该陨坑曾被称作卫星坑[[霍曼陨石坑|霍曼 T]]（所谓的卫星坑标记法：以所靠近的主坑名+字母来命名）。

==参引资料==
{{Reflist}}

==另请参阅==
{{refbegin}}
* {{cite book
  | last1 = Andersson
  | first1 = L. E.
  | last2 = Whitaker
  | first2 = E. A.
  | authorlink2 = Ewen Whitaker
  | date = 1982
  | title = [[NASA|NASA]] Catalogue of Lunar Nomenclature
  | publisher = NASA RP-1097
  | ref = harv
  }}
* {{cite web
  | last = Blue
  | first = Jennifer
  | date = July 25, 2007
  | title = Gazetteer of Planetary Nomenclature
  | publisher = [[United_States_Geological_Survey|USGS]]
  | url = http://planetarynames.wr.usgs.gov/
  | accessdate = 2007-08-05
  | ref = harv
  }}
* {{cite book
  | last1 = Bussey
  | first1 = B.
  | authorlink1 = Ben Bussey
  | last2 = Spudis
  | first2 = P.
  | authorlink2 = Paul Spudis
  | date = 2004
  | title = The Clementine Atlas of the Moon
  | publisher = Cambridge University Press
  | location = New York
  | isbn = 978-0-521-81528-4
  | ref = harv
  }}
* {{cite book
  | last1 = Cocks
  | first1 = Elijah E.
  | last2 = Cocks
  | first2 = Josiah C.
  | date = 1995
  | title = Who's Who on the Moon: A Biographical Dictionary of Lunar Nomenclature
  | publisher = Tudor Publishers
  | isbn = 978-0-936389-27-1
  | ref = harv
  }}
* {{cite web
  | last = McDowell
  | first = Jonathan
  | date = July 15, 2007
  | url = http://host.planet4589.org/astro/lunar/
  | title = Lunar Nomenclature
  | publisher = Jonathan's Space Report
  | accessdate = 2007-10-24
  | ref = harv
  }}
* {{cite journal| last1 = Menzel| first1 = D. H.| last2 = Minnaert| first2 = M.| last3 = Levin| first3 = B.| last4 = Dollfus| first4 = A.| last5 = Bell| first5 = B.| title = Report on Lunar Nomenclature by the Working Group of Commission 17 of the IAU| doi = 10.1007/BF00171763| journal = Space Science Reviews| volume = 12| issue = 2| pages = 136–186| date = 1971| bibcode = 1971SSRv...12..136M| ref = harv}}
* {{cite book
  | first = Patrick
  | last = Moore
  | authorlink = Patrick Moore
  | date = 2001
  | title = On the Moon
  | publisher = Sterling Publishing Co.
  | isbn = 978-0-304-35469-6
  | ref = harv
  }}
* {{cite book
  | first = Fred W.
  | last = Price
  | date = 1988
  | title = The Moon Observer's Handbook
  | publisher = Cambridge University Press
  | isbn = 978-0-521-33500-3
  | ref = harv
  }}
* {{cite book
  | last = Rükl
  | first = Antonín
  | authorlink = Antonín Rükl
  | date = 1990
  | title = Atlas of the Moon
  | publisher = Kalmbach Books
  | isbn = 978-0-913135-17-4
  | ref = harv
  }}
* {{cite book
  | last = Webb
  | first = Rev. T. W.
  | authorlink = Thomas William Webb
  | date = 1962
  | title = Celestial Objects for Common Telescopes
  | edition = 6th revised
  | publisher = Dover
  | isbn = 978-0-486-20917-3
  | ref = harv
  }}
* {{cite book
  | first = Ewen A.
  | last = Whitaker
  | authorlink = Ewen Whitaker
  | date = 1999
  | title = Mapping and Naming the Moon
  | publisher = Cambridge University Press
  | isbn = 978-0-521-62248-6
  | ref = harv
  }}
* {{cite book
  | first = Peter T.
  | last = Wlasuk
  | date = 2000
  | title = Observing the Moon
  | publisher = Springer
  | isbn = 978-1-85233-193-1
  | ref = harv
 }}
{{refend}}

==外部链接==
*[http://www.lpi.usra.edu/resources/lunar_orbiter/bin/srch_nam.shtml?Il%27in%7C0 月球数码摄影图集]
*[http://the-moon.wikispaces.com/Il%27in 维基月球在线解说-伊林陨石坑]
* [http://planet4589.org/astro/lunar/RP-1097.pdf Andersson, L.E., and E.A. Whitaker, 美国宇航局月球地名目录, 美国宇航局参考出版物1097, 1982年10月]

[[Category:月球环形山|I]]