{{Numbers}}
'''無理數'''是指除[[有理数|有理数]]以外的[[实数|实数]]，當中的「理」字来自于[[拉丁语|拉丁语]]的rationalis，意思是「理解」，实际是拉丁文对于logos「说明」的翻译，是指无法用两个[[整数|整数]]的比来说明一个无理数。

非[[有理數|有理數]]之[[實數|實數]]，不能寫作兩整數之比。若將它寫成[[小數|小數]]形式，小數點之後的數字有無限多個，並且不會循環，即无限不循环小数。常見的無理數有大部分的[[平方根|平方根]]、[[圓周率|π]]和[[e_(数学常数)|e]]（其中後兩者同時為[[超越數|超越數]]）等。無理數的另一特徵是無限的[[連分數|連分數]]表達式。

[[傳說|傳說]]中，无理数最早由[[畢達哥拉斯|畢達哥拉斯]]學派弟子[[希伯斯|希伯斯]]发现。他以幾何方法證明<math>\sqrt{2}</math>無法用[[整数|整数]]及[[分數|分數]]表示。而畢達哥拉斯深信任意数均可用整数及分数表示，不相信無理數的存在。後來希伯斯触犯学派章程，将无理数透露给外人，因而被扔进海中处死，其罪名竟然等同于“渎神”。另見[[第一次數學危機|第一次數學危機]]。

無理數可以通過有理數的[[分划|分划]]的概念進行定義。

== 举例 ==
#<math>\sqrt{3}=1.73205080\cdots</math>

#<math>\log_{10}3=0.47712125\cdots</math>

#<math>\pi=3.141592653589793238462643383279502884\cdots</math>

#<math>e=2.71828182845904523536\cdots</math>

#<math>\sin{45^\circ}=\frac \sqrt 2{2}=0.70710678\cdots</math>
...
無理數與無理數的[[四則運算|四則運算]]的結果往往不知道是否無理數，只有<math>\pi+(-\pi)</math>、<math>\sqrt{2}+\sqrt{3}</math>等除外。

我們亦不知道<math>2^e \,</math>、<math>\pi^e \,</math>、<math>\pi^\sqrt{2}</math>、[[欧拉-马歇罗尼常数|欧拉-马歇罗尼常数]]<math>\gamma \,</math>或[[卡塔兰常数|卡塔兰常数]]<math>G</math>是否無理數。

== 無理數集的特性 ==

無理數集是[[不可數集|不可數集]]（因有理數集是[[可數集|可數集]]而實數集是不可數集）。無理數集是個不[[完備|完備]]的[[拓撲空間|拓撲空間]]，它是與所有[[正數|正數]][[數列|數列]]的集[[拓撲同構|拓撲同構]]的，當中的同構[[映射|映射]]是無理數的[[連分數|連分數]]開展。因而[[贝尔纲定理|贝尔纲定理]]可以應用在無數間的拓撲空間上。

== 無理化作連分數的表達式 ==
:<math>x^2=c\qquad(c>0)</math>，
選取一個正的實數<math>\rho\,</math>使得
:<math>\rho^2<c\!</math>。
經由遞迴處理
:<math>\begin{align}
x^2\ -\!\rho^2&=c\ -\!\rho^2\\
...
假设<math>\sqrt{2}</math>是有理数，并且令<math>\sqrt{2}=\frac{p}{q}</math>，<math>\frac{p}{q}</math>是最简分数。

两边平方，得到<math>2=\frac{p^2}{q^2}</math>。将此式改写为<math>2q^2=p^2</math>，可见<math>p^2</math>为偶数。

因为平方运算保持奇偶性，所以<math>p</math>只能为偶数。设<math>p=2p_1</math>，其中<math>p_1</math>为整数。

代入可得<math>q^2=2p_1^2</math>。同理可得<math>q</math>亦为偶数。

这与<math>\frac{p}{q}</math>为最简分数的假设矛盾，
所以<math>\sqrt{2}</math>是有理数的假设不成立。

=== 證明<math>\sqrt{2}+\sqrt{3}</math>是无理数 ===

'''證：'''

假設<math>\sqrt{2}+\sqrt{3}=p</math>是有理數，兩邊平方得到

<math>5+2\sqrt{6}=p^2\Rightarrow\sqrt{6}=\frac{p^2-5}{2}</math>

其中因為<math>p</math>是有理數，所以<math>\frac{p^2-5}{2}</math>也是有理數。

透過證明<math>\sqrt{a}</math>為無理數的方法，其中<math>{a}</math>為一整數
...
同樣，假設<math>\sqrt{2}+\sqrt{3}+\sqrt{5}=p</math>是有理數，兩邊平方後得到
...
透過證明形如<math>\sqrt{a}+\sqrt{b}</math>的數是無理數的方法，得出<math>\sqrt{6}+p\sqrt{5}</math>是一無理數
...
<math>\Rightarrow\sqrt{2}+\sqrt{3}+\sqrt{5}=p-\sqrt{7}</math>，兩邊平方得到：
...
透過證明形如<math>\sqrt{a}+\sqrt{b}+\sqrt{c}</math>的數是無理數的方法，可知<math>\sqrt{10}+6\sqrt{6}+p(2p^2-3)\sqrt{7}</math>為無理數

這和<math>q</math>是有理數衝突

所以得證<math>\sqrt{2}+\sqrt{3}+\sqrt{5}+\sqrt{7}</math>為一無理數
...
* [http://www.math.ust.hk/excalibur/v3_n1.pdf <math>\sqrt{2}</math>是無理數的六個證明，香港大學數學系蕭文強]（Mathematical Excalibur Vol.3 No.1 Page 2）
* [http://www.math.sinica.edu.tw/math_media/pdf.php?m_file=ZDMwNC8zMDQwNA==%20 舊題新解—根號2是無理數，張海潮 張鎮華]{{dead link|date=2018年1月 |bot=InternetArchiveBot |fix-attempted=yes }}（數學傳播 第30卷 第4期）
