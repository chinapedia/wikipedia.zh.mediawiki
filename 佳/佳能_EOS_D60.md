{{Infobox Digicam |
  model   = [[Canon_EOS|Canon EOS]] D60 |
  image = [[Image:Canon_D60_brighterorange.jpg|250px]]|
  kind   = [[Single-lens_reflex_camera|Single-lens reflex]] |
  sensor   = [[CMOS|CMOS]]|
  res = 3,072 x 2,048 (6.3 [[megapixels|megapixels]])|
  lens = Interchangeable ([[Canon_EF_lens_mount|EF]])|
  viewfinder = Optical|
  storage = [[CompactFlash|CompactFlash]] (Type I or Type II) / max. 2GiB|
  shutter = electronic focal-plane |
  shutterRange = 30 to 1/4000 s |
  metering = TTL, full aperture, zones |
  emode = Full auto, programmed, shutter-priority, aperture priority, manual |
  mmode = Evaluative, Partial, Center Weighted |
  farea = 3 focus points |
  fmode = One-shot, AI Servo, AI-Focus, Manual |
  cont = up to 3.0 frame/s |
  speedRange = 100-1000 in 1 EV steps|
  rearLCD = 1.8 in (46 mm), 114,000 pixels |
  flbkt = none |
  fcbkt = none |
  WB = 7 presets, Auto and custom |
  wbbkt = none |
  flash = pop-up |
  weight = 780 g (body only) |
  battery = Li-Ion BP-511 rechargeable|
  obp = BP-511A, [[Canon_BG-ED3|BG-ED3]] |
  madeIn = [[Japan|Japan]]|
}}
'''佳能 EOS D60'''是[[佳能|佳能]]于2002年2月22日推出的一款630万[[像素|像素]][[数码单镜反光相机|数码单镜反光相机]]，支持[[佳能EF接环镜头|佳能EF接环镜头]]，但是不支持佳能后来研发的[[佳能EF-S接环镜头|佳能EF-S接环镜头]]。

== 外部链接 ==
{{commonscat-inline|Canon EOS D60}}
* [http://www.usa.canon.com/consumer/controller?act=ModelInfoAct&fcategoryid=215&modelid=7495 Product Page]
* [http://www.dpreview.com/reviews/canoneosd60/ dpreview.com's Canon EOS D60 Review]

{{Canon DSLR cameras}}

[[Category:佳能相机|Category:佳能相机]]