{{Unreferenced|time=2017-12-26T09:55:01+00:00}}
以下是部份無理函數的[[积分表|积分表]].
...
: <math>\int r^5 \; dx = \frac{1}{6}xr^5+\frac{5}{24}a^2xr^3+\frac{5}{16}a^4xr+\frac{5}{16}a^6\ln\left(\frac{x+r}{a}\right)</math>
...
: <math>\int\frac{x^2\;dx}{r} = \frac{x}{2}r-\frac{a^2}{2}\,\sinh^{-1}\frac{x}{a} = \frac{x}{2}r-\frac{a^2}{2}\ln\left|x+r\right|</math>

: <math>\int\frac{dx}{xr} = -\frac{1}{a}\,\sinh^{-1}\frac{a}{x} = -\frac{1}{a}\ln\left|\frac{a+r}{x}\right|</math>

== 包括 ''s'' = √<span style="text-decoration:overline">''x''<sup>2</sup> - ''a''<sup>2</sup></span>的积分==
假設 <math>(x^2>a^2)</math>, <math>(x^2<a^2)</math>請看下一節:
...
注意在<math>\ln\left|\frac{x+s}{a}\right|
=\mathrm{sgn}(x)\cosh^{-1}\left|\frac{x}{a}\right|
=\frac{1}{2}\ln\left(\frac{x+s}{x-s}\right)</math>, 里<math>\cosh^{-1}\left|\frac{x}{a}\right|</math>取正值.
...
: <math>\int t\;dx = \frac{1}{2}\left(xt-\sgn x\,\cosh^{-1}\left|\frac{x}{a}\right|\right) \qquad\mbox{(}|x|\ge|a|\mbox{)}</math>

== 包括√<span style="text-decoration:overline">R</span> = √<span style="text-decoration:overline">''ax''<sup>2</sup> + ''bx'' + ''c''</span>的积分==
: <math>\int\frac{dx}{\sqrt{ax^2+bx+c}} = \frac{1}{\sqrt{a}}\ln\left|2\sqrt{a R}+2ax+b\right| \qquad\qquad \mbox{(}a>0\mbox{)}</math>
: <math>\int\frac{dx}{\sqrt{ax^2+bx+c}} = \frac{1}{\sqrt{a}}\,\sinh^{-1}\frac{2ax+b}{\sqrt{4ac-b^2}} \qquad\qquad \mbox{(}a>0\mbox{, }4ac-b^2>0\mbox{)}</math>
: <math>\int\frac{dx}{\sqrt{ax^2+bx+c}} = \frac{1}{\sqrt{a}}\ln|2ax+b| \quad \mbox{(}a>0\mbox{, }4ac-b^2=0\mbox{)}</math>
: <math>\int\frac{dx}{\sqrt{ax^2+bx+c}} = -\frac{1}{\sqrt{-a}}\arcsin\frac{2ax+b}{\sqrt{b^2-4ac}} \qquad\qquad \mbox{(}a<0\mbox{, }4ac-b^2<0\mbox{)}</math>
