''一个'''莫尔斯函数'''也是一个[[非简谐振子|非简谐振子]]的一种表达法''。

在[[微分拓扑|微分拓扑]]中，'''莫尔斯理论'''的技术给出了一个非常直接的分析一个[[流形|流形]]的拓扑的方法，它是通过研究该流形上的[[可微函数|可微函数]]达成。根据莫尔斯的基本见解，一个流形上的一个可微函数在'''''典型'''''的情况下，很直接的反映了该流形的拓扑。莫尔斯理论允许人们在流形上找到[[CW复形|CW结构]]和[[柄分解|柄分解]]，并得到关于它们的[[同调群|同调群]]的信息。在莫尔斯之前，[[凯莱|凯莱]]和[[麦克斯韦|麦克斯韦]]在[[制图学|制图学]]的情况下发展了莫尔斯理论中的一些思想。莫尔斯最初将他的理论用于[[测地线|测地线]]（路径的能量函数的临界点）。这些技术被[[拉乌尔·博特|拉乌尔·博特]]用于他的著名的[[博特周期性定理|博特周期性定理]]的证明中。

==参看==
*[[萨德引理|萨德引理]]
*[[Lyusternik-Schnirelmann类|Lyusternik-Schnirelmann类]]

==进阶阅读==
* Milnor, John (1963). Morse Theory
* Matsumoto, Yukio (2002). An Introduction to Morse Theory
* Morse, Marston (1934). The Calculus of Variations in the Large
* Seifert, Herbert & Threlfall, William (1938). Variationsrechnung im Grossen
* Bott, Raul (1988) [http://www.numdam.org/item?id=PMIHES_1988__68__99_0 Morse Theory Indomitable.] ''Publications Mathématiques de l'IHÉS.'' '''68''', 99-114.
* Milnor, John (1965). Lectures on the [[h-Cobordism_theorem|h-Cobordism theorem]] - scans available [http://www.maths.ed.ac.uk/~aar/surgery/hcobord.pdf here]
* Maxwell, James Clerk (1870).  [http://www.maths.ed.ac.uk/~aar/surgery/hilldale.pdf On Hills and Dales.] ''The Philosophical Magazine'' '''40''' (269), 421-427. 
* Cayley, Arthur (1859). On Contour and Slope Line. ''The Philosophical Magazine'' '''18''' (120), 264-268.

[[Category:莫尔斯理论|*]]
[[Category:引理|Category:引理]]
[[Category:微分拓扑学|Category:微分拓扑学]]
[[Category:光滑函数|Category:光滑函数]]