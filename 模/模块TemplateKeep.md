            ...
            if short then
                ...
                if tinfo == nil then
                    ...
                    for k, aname in ipairs( allnames ) do
                        table.insert( tusage, '{{tl|k|' .. aname .. '}}' )
                    end
    ...
    while i < 10 do
        ...
        if arg and map[mw.text.trim( arg:lower() )] then
            ...
            if check then
                ...
                if frame.args.deletelink then
                    table.insert( deletelinks, '[[WP:快速保留的标准#'_.._item[1]_.._'|' .. item[1] .. ']]: ' .. item[3] .. deletesuffix )
                end
                rowsuffix = args.cat or args.cate or args.category or ( '[[Category:快速保留候选|' .. ( item[7] or '速' ) .. ']]' )
            end
            row = '* <span id="speedy-delete-' .. item[1] .. '" title="' .. extractShortDesc( item ) .. '">' .. item[3] .. '（[[WP:快速保留的标准#'_.._item[1]_.._'|CSK ' .. item[1] .. ']]）' .. rowsuffix .. rowsuffix2
        ...
        elseif arg and mw.text.trim( arg ) ~= '' then
            ...
            if title and title.exists then
                if frame.args.deletelink then
                    table.insert( deletelinks, '[[WP:快速保留的标准|CSK]]: [[:'_.._arg_.._'|:' .. arg .. ']]' )
                end
                table.insert( rows, '* ' .. cat .. '[[:'_.._arg_.._'|:' .. arg .. ']]' )
            else
                if frame.args.deletelink then
