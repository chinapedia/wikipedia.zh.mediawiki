{{noteTA|G1=IT}}
'''模板'''（'''{{lang|en|Template}}'''）指[[C++|C++]][[程序设计语言|程序设计语言]]中的[[子程序|函数]]模板与[[类_(计算机科学)|类]]模板<ref name="msdn">[http://msdn.microsoft.com/zh-cn/library/71dw8xzh.aspx MSDN：嵌套的类模板]</ref>，是一种[[参数化类型|参数化类型]]机制，大体对应于[[java|java]]和[[C＃|C＃]]中的[[泛型|泛型]]，但也有一些功能上的显著差异（C++模板支持后两者没有明确对应的模板模板参数和模板非类型参数，但不支持Java的通配符以及C#的泛型类型约束）。模板是C++的[[泛型编程|泛型编程]]中不可缺少的一部分。

模板是C++程序员绝佳的武器，特別是結合了[[多重继承|多重继承]]与[[运算符重载|运算符重载]]之后。C++的标准函数库提供的许多有用的函数大多結合了模板的概念，如[[STL|STL]]以及[[iostream|iostream]]。
==语法==
===模板的声明与定义===
...
[[gcc|g++]]的模板实例化，目前分为三种方式：<ref>{{cite web|url=https://gcc.gnu.org/onlinedocs/gcc/Template-Instantiation.html#Template-Instantiation|title=Template Instantiation|accessdate=2014-09-27}}</ref>
