'''弗洛凱理論'''是[[常微分方程|常微分方程]]理論的一種，討論有關下列[[微分方程|微分方程]]類型的解答類別，

:<math>\dot{x} = A(t) x</math>,

其中，''A(t)''是一週期為''T''的連續週期函數。

弗洛凱理論的主要定理－'''弗洛凱定理'''給出了一般[[線性系統|線性系統]]的每個[[基本解|基本解]]的[[正規形式|正規形式]]。它給定了一[[局部坐標|座標轉變]]<math>y=Q^{-1}(t)x</math>，其中<math>Q(t+2T)=Q(t)</math>，用以來轉變[[週期系統|週期系統]]至有常數及實[[係數|係數]]的傳統線性系統。

在[[固態物理|固態物理]]中，其類比的結果(推廣至三維)為[[布洛赫定理|布洛赫定理]]。
...
弗洛凯理论的主要定理－弗洛凯定理给出了一般线性系统的每个基本解的正规形式。它给定了一座标转变<math>y=Q^{-1}(t)x</math>，其中<math>Q(t+2T)=Q(t)</math>，用以来转变[[周期系统|周期系统]]至有常数及实系数的传统[[线性系统|线性系统]]。

在[[固态物理|固态物理]]中，其类比的结果（推广至三维）为布洛赫定理。
