...
{{multiple issues|
{{copyedit|time=2013-03-12T14:05:49+00:00}}
{{no footnotes|time=2013-03-12T14:05:49+00:00}}
{{untranslated-jargon|time=2013-04-21}}
}}
{{noteTA
|G1=Communication
|1=zh-hans:复用; zh-hant:多工;
|2=zh-hans:基带; zh-hant:基频;
}}
{{Modulation techniques}}
[[File:OFDMの周波数.PNG|thumb]]
[[File:dvbt_spectrum.jpg|thumb]]
'''正交頻分多工'''（{{lang-en|Orthogonal frequency-division multiplexing, OFDM}}）有時又稱為'''分離複頻調變技術'''（{{lang-en|discrete multitone modulation, DMT}}），可以視為多載波傳輸的一個特例，具備高速率資料傳輸的能力，加上能有效對抗[[衰落|頻率選擇性衰減]]，而逐漸獲得重視與採用。

OFDM使用大量緊鄰的正交[[子載波|子載波]]（Orthogonal sub-carrier），每個子載波採用傳統的調變方案，進行低符號率調製。可以視為一調變技術與多工技術的結合。

==定義==
[[調變|調變]]是將傳送資料對應於載波變化的動作，可以是載波的相位、頻率、振幅、或是其組合。正交頻分複用之基本觀念為將一高速資料流程，分割成數個低速資料流程，並將這數個低速資料流程同時調製在數個彼此相互正交載波上傳送。由於每個子載波頻寬較小，更接近於相干頻寬，故可以有效對抗頻率選擇性衰弱，因此現今以大量採用於無線通訊。
...
輸出：<math>y[n] = \sum_{j=0}^{L-1} h_{l} x[n-l] + z[n]=h_{0} x[n] + \sum_{j=1}^{L-1} h_{l} x[n-l] + z[n]</math>

其中<math>h_0 x[n]</math>是我們想要的訊號，<math>\sum_{j=1}^{L-1} h_{l} x[n-l]</math>是過去的訊號對現在所得訊號的干擾，也就是所謂的符元間干擾(Inter-symbol Interference)，<math>z[n]</math>是雜訊。L 可以視為線性非時變系統下tap的個數。

傳統上，我們可以透過維特比演算法(Viterbi Algorithm)，一種動態規劃的演算法，來找尋最有可能序列探測可能性(Maximum Likelihood Sequence Detection)，但這種演算法的時間複雜度為<math>O(2^L)</math>，在現在寬頻的系統下，L的大小大約從100至400，這樣的複雜度對於現在系統並不是非常經濟的。為此，我們引進了OFDM系統。

=== 數學推導 ===
OFDM 是一個避免ISI的數學架構，以下說明他如何避免ISI，在一個OFDM系統下
<math>\{\tilde{x_k}\} \rightarrow pre-processing \rightarrow \{x[n]\} \rightarrow h_l \rightarrow \{y[n]\} \rightarrow post-processing \rightarrow \{\tilde{y_k}\}</math>

我們希望可以做到
<math>\tilde{y_k} = \tilde{h_k} \tilde{x_k}</math>
，這樣就是一個ISI-free的架構
若我們考慮線性旋積(linear convolution)，我們想要證明時域上的線性旋積是頻域上的乘法，我們可以用
<math>\hat{x}(f) \triangleq \sum_{n=-\infty}^{\infty} x[n] e^{-j2\pi fn}</math>

<math>\hat{h}(f) \triangleq \sum_{n=-\infty}^{\infty} h[n] e^{-j2\pi fn}</math>

<math>y[n]=\left( h \ast x \right) [n] = \sum_{k=-\infty}^{\infty} h[k]x[n-k]</math>

<math>\hat{y}(f) = \hat{h}(f) \cdot \hat{x}(f)</math>

從推導我們可以發現證明時域上線性旋積等價於頻域上的乘法，是構築在積分範圍從<math>-\infty</math>至<math>\infty</math>，但在離散時間的傅立葉轉換中，其積分範圍為有限值，但我們仍然想要保有時域上的旋積等價於頻域上的乘法，故為此我們引進了環旋積(circular convolution)。
*環旋積(circular convolution)
<math>\left( h \circledast_{N} x \right) = \sum_{k=0}^{N-1} x[ {\left( n-k \right)}_{mod\ N}] h[k] </math>

我們可以透過輸入、線性非時變系統脈衝響應的離散傅立葉轉換以及環旋積的定義：

<math> \hat{x_k} \triangleq \frac{1}{\sqrt{N}} \sum_{n=0}^{N-1} x[n] e^{-j2 \pi \frac{k}{N} n} </math>

<math> \hat{h_k} \triangleq \frac{1}{\sqrt{N}} \sum_{n=0}^{N-1} h[n] e^{-j2 \pi \frac{k}{N} n} </math>

<math> y[n] = \left( h \circledast_{N} x \right) = \sum_{k=0}^{N-1} x[ {\left( n-k \right)}_{mod\ N}] h[k] </math>

來導出對於離散傅立葉轉換下，時域上的環旋積等價於頻域上的乘法。

<math> \hat{y_l} = \sqrt{N} \cdot \hat{h_k} \cdot \hat{x_k} </math>

因此，我們可以將架構看成：

<math> \{  \hat{x_k} \} \rightarrow  IDFT \rightarrow \{ x[n] \} \rightarrow circular\ convolution\ h \rightarrow  \{ y[n] \} \rightarrow DFT \rightarrow \{ \hat{y_k} \} </math>
...
=== 物理理解 ===
OFDM 系統與傳統分頻多工(Frequency Division Multiplexing， FDM)不同之 處為:OFDM 系統的每個子載波(Sub-Carrier)之間具有正交性(Orthogonality)，此 為 OFDM 系統之主要特點。子載波彼此之間不會產生干擾，且頻譜可以相互重疊;而傳統分頻多工系統之不同載波之間頻譜沒有重疊，因此 OFDM 系統比傳 統分頻多工系統具有較好的頻寬效益(Bandwidth Efficiency)。

那我們如何得出子載波彼此之間都相互正交的性質呢？

OFDM引進了[[快速傅里叶逆变换|快速傅里叶逆变换]]（IFFT）和[[快速傅里叶变换|快速傅里叶变换]]（FFT），他們兩個都是正交轉換(orthogonal transfrom)，所以我們可以視為在頻譜上先擺好資訊，而這些資訊們因為透過IFFT轉換成時域，而他使用的载波（carrier）必定正交，因為IFFT的每個[[基_(线性代数)|基]]（basis）都相互正交，可以視為其他子载波（subcarrier）對自己子载波沒有影響，這樣的優點在於頻譜可以重疊而不互相干擾，是OFDM優點之一。
...
計算反FFT或FFT變換的時間必須少於每個符號的時間，例如對於DVB-T (FFT 8k) 意味著計算必須在896 <math>\mu s</math> 或更短的時間內完成。

對於8192點FFT，這可以近似為：

<math>MIPS = \frac{computational complexity}{T_symbol} \times 1.3 \times 10^{-6}</math>

<math>= \frac{147 456\times 2}{896 \times 10^{-6}} \times 1.3 \times 10^{-6}</math>

<math>= 428</math> 

* MIPS = Million instructions per second

計算時間大致與FFT大小成線性關係，因此雙倍大小的FFT需要兩倍的時間，反之亦然。作為比較，1.266 GHZ的Intel Pentium III CPU能夠使用FFTW在576<math>\mu s</math>計算出8192點FFT。1.6 GHz的Intel Pentium M可以在387<math>\mu s</math>內完成。Intel Core Duo 3.0 GHz可以在96.8<math>\mu s</math>內完成。
...
一個簡單的例子：上述數值例子中的OFDM等化需要每個子載波和符號一個複數值乘法（也就是，每個OFDM符號  N = 1000次複數乘法;也就是在接收端每秒一百萬次乘法）FFT算法需要<math>N \log_2 N = 10000</math>[這是不精確的：這些複雜的一半以上乘法是微不足道的，也就是等於不在軟體中實現]。每個OFDM符號的複數值乘法（也就是每秒一千萬次乘法），在接收端和發射端。這應該與此例中提到的相應的100萬符號/秒單載波調變情況進行比較，其中使用FIR濾波器進行125微秒時間擴張的等化在初始實現中需要每個符號125次乘法（也就是每秒1.25億次乘法）。FFT技術可以用於將基於FIR濾波器的時域等化器的乘法數量減少到與OFDM相當的數量，其代價是接收和解碼之間的延遲也與OFDM相當。
...
[[File:OFDM_transmitter_ideal.png|center]]

=== 接收 ===

[[File:OFDM_receiver_ideal.png|center]]
<!--
The receiver picks up the signal <math>r(t)</math>, which is then quadrature-mixed down to baseband using cosine and sine waves at the carrier frequency. This also creates signals centered on <math>2 f_c</math>, so low-pass filters are used to reject these. The baseband signals are then sampled and digitised using [[analogue-to-digital_converter|analogue-to-digital converter]]s (ADCs), and a forward FFT is used to convert back to the frequency domain.

This returns <math>N</math> parallel streams, each of which is converted to a binary stream using an appropriate symbol [[detection|detector]]. These streams are then re-combined into a serial stream, <math>{\hat s}[n]</math>, which is an estimate of the original binary stream at the transmitter.
-->
...
<!-- 4rNCHbWexLMBbXgBd9gzLbvyNv2CaeHb3oNStNQ+fNoasaacH8YjY= -->
<!-- vipgYlh9vqqj=hEeeu0xXdbba9frFj0=OqFfea0dXdd9vqai=hGuQ8 -->
<!-- kuc9pgc9q8qqaq=dir=f0=yqaiVgFr0xfr=xfr=xb9adbaqaaeGaci -->
<!-- GaaiaabeqaamaabaabauaakeaacaWG4bWaaSbaaSqaaiaad2gaaeqa -->
<!-- aOGaaiikaiaadshacaGGPaGaeyypa0Jaci4yaiaac+gacaGGZbGaai -->
<!-- ikaiaaikdacqaHapaCcaGGOaGaamOzamaaBaaaleaacaWGJbaabeaa -->
<!-- kiabgUcaRiaadAgadaWgaaWcbaGaamyBaaqabaGccaGGPaGaamiDai -->
<!-- aacMcacqGH9aqpciGGsbGaaiyzaiaacIcacaWGLbWaaWbaaSqabeaa -->
<!-- caWGQbGaaGOmaiabec8aWjaacIcacaWGMbWaaSbaaWqaaiaadogaae -->
<!-- qaaSGaaiiFaiabgUcaRiaadAgadaWgaaadbaGaamyBaaqabaWccaGG -->
<!-- PaGaamiDaaaakiaacMcacqGH9aqpciGGsbGaaiyzaiaacIcacaWG4b -->
<!-- WaaSbaaSqaaiaadYgacaWGTbaabeaakiaacIcacaWG0bGaaiykaiaa -->
<!-- dwgadaahaaWcbeqaaiaadQgacaaIYaGaeqiWdaNaamOzamaaBaaame -->
<!-- aacaWGJbaabeaaliaadshaaaGccaGGPaaaaa?7747? -->

<math>x_{m}(t)=\cos (2\pi (f_{c}+f_{m})t)=\operatorname{Re}(e^{j2\pi (f_{c}|+f_{m})t})=\operatorname{Re}(x_{lm}(t)e^{j2\pi f_{c}t})</math>
...
<!-- vipgYlh9vqqj=hEeeu0xXdbba9frFj0=OqFfea0dXdd9vqai=hGuQ8 -->
<!-- kuc9pgc9q8qqaq=dir=f0=yqaiVgFr0xfr=xfr=xb9adbaqaaeGaci -->
<!-- GaaiaabeqaamaabaabauaakeGabaayaiaadkfacqGH9aqpdaWdXaqa -->
<!-- aiaadwgadaahaaWcbeqaaiaadQgacaaIYaGaeqiWdaNaamOzamaaBa -->
<!-- aameaacaaIXaaabeaaliaadshaaaGccaGGOaGaamyzamaaCaaaleqa -->
<!-- baGaamOAaiaaikdacqaHapaCcaWGMbWaaSbaaWqaaiaaikdaaeqaaS -->
<!-- GaamiDaaaakiaacMcadaahaaWcbeqaaiaacQcaaaGccaWGKbGaamiD -->
<!-- aiabg2da9aWcbaGaaGimaaqaaiaadsfaa0Gaey4kIipakmaapedaba -->
<!-- GaamyzamaaCaaaleqabaGaamOAaiaaikdacqaHapaCcqGHuoarcaWG -->
<!-- MbGaamiDaaaakiaadsgacaWG0bGaeyypa0ZaaSaaaeaaciGGZbGaai -->
<!-- yAaiaac6gacaGGOaGaeqiWdaNaeyiLdqKaamOzaiaadsfacaGGPaaa -->
<!-- baGaeqiWdaNaeyiLdqKaamOzaaaaaSqaaiaaicdaaeaacaWGubaani -->
<!-- abgUIiYdGccaWGLbWaaWbaaSqabeaacaWGQbGaeqiWdaNaeyiLdqKa -->
<!-- amOzaiaadsfaaaaaaa?7E58? -->

<math>R=\int_{0}^{T}{e^{j2\pi f_{1}t}(e^{j2\pi f_{2}t})^{*}dt=}\int_{0}^{T}{e^{j2\pi \Delta ft}dt=\frac{\sin (\pi \Delta fT)}{\pi \Delta f}}e^{j\pi \Delta fT}</math>
 
其中Δf=<math>f_1-f_2</math>表兩個載波間的頻率間隔，在上式中若ΔfT = n其中n為一個非零整數，如：Δf=n/T則此時R=0即代表這兩個子載波在符元週期內為正交。
...
<!-- vipgYlh9vqqj=hEeeu0xXdbba9frFj0=OqFfea0dXdd9vqai=hGuQ8 -->
<!-- kuc9pgc9q8qqaq=dir=f0=yqaiVgFr0xfr=xfr=xb9adbaqaaeGaci -->
<!-- GaaiaabeqaamaabaabauaakeaacaWGZbGaaiikaiaadshacaGGPaGa -->
<!-- eyypa0ZaaabCaeaacaWGybWaaSbaaSqaaiaadUgaaeqaaOGaamyzam -->
<!-- aaCaaaleqabaGaamOAaiaaikdacqaHapaCcaWGMbWaaSbaaWqaaiaa -->
<!-- dUgaaeqaaSGaamiDaaaaaeaacaWGRbGaeyypa0JaaGimaaqaaiaad6 -->
<!-- eacqGHsislcaaIXaaaniabggHiLdaaaa?5703? -->

<math>s(t)=\sum\limits_{k=0}^{N-1}{X_{k}e^{j2\pi f_{k}t}}</math>

對t=N<math>T_s</math>取樣  <math>s(nT_{s})=\sum\limits_{k=0}^{N-1}{X_{k}e^{j2\pi f_{k}nT_{S}}}</math>

 
取f=<math>1/NT_S</math>,<math>f_k</math>=kf得  <math>x_{n}=s(nT_{s})=\sum\limits_{k=0}^{N-1}{X_{k}e^{\frac{j2\pi kn}{N}}}</math>:IDFT
...
<!-- vipgYlh9vqqj=hEeeu0xXdbba9frFj0=OqFfea0dXdd9vqai=hGuQ8 -->
<!-- kuc9pgc9q8qqaq=dir=f0=yqaiVgFr0xfr=xfr=xb9adbaqaaeGaci -->
<!-- GaaiaabeqaamaabaabauaakeaacaWGzbWaaSbaaSqaaiaadUgaaeqa -->
<!-- aOGaeyypa0ZaaabCaeaacaGGOaWaaabCaeaacaWG4bWaaSbaaSqaai -->
<!-- aad6gacqGHsislcaWGTbGaeyOeI0IaamiBaaqabaGccaWGObWaaSba -->
<!-- aSqaaiaadYgaaeqaaOGaaiykaiaadwgadaahaaWcbeqaaiabgkHiTi -->
<!-- aadQgadaWcaaqaaiaaikdacqaHapaCcaWGRbGaamOBaaqaaiaad6ea -->
<!-- aaaaaaqaaiaadYgacqGH9aqpcaaIWaaabaGaamitaaqdcqGHris5aO -->
<!-- Gaeyypa0JaamiwamaaBaaaleaacaWGRbaabeaakiaadIeadaWgaaWc -->
<!-- baGaam4AaaqabaGccaWGLbWaaWbaaSqabeaadaWcaaqaaiabgkHiTi -->
<!-- aadQgacaaIYaGaeqiWdaNaam4Aaiaad6gaaeaacaWGobaaaaaaaeaa -->
<!-- caWGUbGaeyypa0JaaGimaaqaaiaad6eacqGHsislcaaIXaaaniabgg -->
<!-- HiLdaaaa?70A5? -->

:<math>Y_{k}=\sum\limits_{n=0}^{N-1}{(\sum\limits_{l=0}^{L}{x_{n-m-l}h_{l})e^{-j\frac{2\pi kn}{N}}}=X_{k}H_{k}e^{\frac{-j2\pi kn}{N}}}</math>
...
== 参见 ==
* [[频率分集|频率分集]]
* [[Chirp_spread_spectrum|Chirp spread spectrum]]
...
== 外部連結 ==
* [http://www.supelec.fr/d2ri/flexibleradio/cours/ofdmtutorial.pdf OFDM简介]{{en}}[http://www.supelec.fr/d2ri/flexibleradio/cours/cours-ofdm-french.pdf OFDM简介]{{fr}}[http://www.supelec.fr/d2ri/flexibleradio/cours/precoders-OFDM.pdf OFDM系统中的线性预编码技术]{{en}}- Tutorial written by Prof. Debbah, head of the Alcatel-Lucent Chair on flexible radio
* [http://wcsp.eng.usf.edu/OFDM_links.html Numerous useful links and resources for OFDM] - WCSP Group - University of South Florida (USF)
* [http://www.wimaxforum.org WiMAX Forum, WiMAX, the framework standard for 4G mobile personal broadband]
* [http://www.flarion.com/products/flash_ofdm.asp Flarion Technologies, the inventor of FLASH-OFDM]{{dead link|date=2018年3月 |bot=InternetArchiveBot |fix-attempted=yes }}
* [http://www.qualcomm.com/about/flarion.html QUALCOMM, parent company of Flarion Technologies]{{dead link|date=2018年3月 |bot=InternetArchiveBot |fix-attempted=yes }}
* Stott, 1997 [http://www.bbc.co.uk/rd/pubs/papers/paper_15/paper_15.shtml] Technical presentation by J H Stott of the BBC's R&D division, delivered at the 20 International Television Symposium in 1997; this URL accessed [[24_January|24 January]] 2006.
* Page on Orthogonal Frequency Division Multiplexing at http://www.iss.rwth-aachen.de/Projekte/Theo/OFDM/node6.html accessed on [[24_September|24 September]] 2007.
* [https://web.archive.org/web/20060525045608/http://www.infosyncworld.com/news/n/5345.html Siemens demos 360 Mbit/s wireless]
* [http://www.pattools.com/cgi-bin/patent_navigator.pl?patent=5282222 1994 US Patent 5,282,222 for wireless data transmission]{{dead link|date=2018年3月 |bot=InternetArchiveBot |fix-attempted=yes }} - The patent "tree" rooted on this patent has upwards of 20000 nodes and leaves references.
* [https://web.archive.org/web/20110722145323/http://www.ieee.li/pdf/viewgraphs/introduction_orthogonal_frequency_division_multiplex.pdf An Introduction to Orthogonal Frequency Division Multiplex Technology]
