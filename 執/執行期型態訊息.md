{{noteTA
|G1=IT
|T=zh-cn:运行期类型信息;zh-tw:執行期型態訊息
|1=zh-cn:类型;zh-tw:型態
|2=zh-cn:信息;zh-tw:訊息
}}
在程式設計中，所謂的'''執行期型態訊息'''（Runtime type information，RTTI）指的是在程式[[執行期|執行]]時保存其物件的[[資料型態|型態]]訊息的行為。某些語言實作僅保留有限的型態訊息，例如[[面向对象程序设计#继承性|繼承樹]]資訊，而某些實作會保留較多資訊，例如物件的屬性及[[面向对象程序设计#方法|方法]]訊息。
