{{noteTA
|T=zh:高氯酸銫;zh-hk:高氯酸銫;zh-hans:高氯酸铯; zh-tw:過氯酸銫;
|1=zh-hk:高氯酸;zh-hans:高氯酸; zh-tw:過氯酸;
|2=zh:鈁; zh-hans:钫; zh-hant:鍅;
|3=zh:钫; zh-hans:钫; zh-hant:鍅;
|4=zh:鍅; zh-hans:钫; zh-hant:鍅;
|5=zh-hans:砹;zh-hk:砈;zh-tw:砈;
}}
{{chembox
| verifiedrevid = 396306603
| Reference = <ref name="perry">{{citation | first1 = Dale L. | last1 = Perry | first2 = Sidney L. | last2 = Phillips | title = Handbook of Inorganic Compounds | publisher = CRC Press | year = 1995}}.</ref><ref name="chtas">{{citation | first1 = F. | last1 = Brezina | first2 = J. | last2 = Mollin | first3 = R. | last3 = Pastorek | first4 = Z. | last4 = Sindelar | title = Chemicke tabulky anorganickych sloucenin | publisher = SNTL | year = 1986}}.</ref>
| ImageFile = cesium perchlorate.png
| ImageSize = 150px
| ImageFile1 = Cesium perchlorate 25g.jpg 
| IUPACName = Caesium perchlorate
| OtherNames = 高氯酸銫
| Section1 = {{Chembox Identifiers
|   InChI = 1/ClHO4.Cs/c2-1(3,4)5;/h(H,2,3,4,5);/q;+1/p-1
|   SMILES = [Cs+].[O-]Cl(=O)(=O)=O
| InChIKey = WKDKOOITVYKILI-REWHXWOFAB
| StdInChI_Ref = {{stdinchicite|correct|chemspider}}
| StdInChI = 1S/ClHO4.Cs/c2-1(3,4)5;/h(H,2,3,4,5);/q;+1/p-1
| StdInChIKey_Ref = {{stdinchicite|correct|chemspider}}
| StdInChIKey = WKDKOOITVYKILI-UHFFFAOYSA-M
| CASNo_Ref = {{cascite|correct|??}}
| CASNo = 13454-84-7
|   EINECS = 236-643-0
|   ChemSpiderID_Ref = {{chemspidercite|correct|chemspider}}
| ChemSpiderID = 109912
|   PubChem = 3035378
  }}
| Section2 = {{Chembox Properties
|   Formula = CsClO<sub>4</sub>
|   MolarMass = 232.36 g/mol
|   Appearance = 無色結晶
|   Density = 3.327 g/cm<sup>3</sup>
|   MeltingPt = 250 °C 分解.
|   Solubility = 1.974 g/100 ml (25 °C)
  }}
| Section3 = {{Chembox Structure
|   CrystalStruct =  [[斜方晶系|斜方晶系]] (<219 °C)<br/>[[等軸晶系|立方晶系]] (>219 °C, ''a'' = 798 pm)
|   SpaceGroup = Pnma (<219 °C)<br/>F43m (>219 °C)
|   LattConst_a = 982 pm
|   LattConst_b = 600 pm
|   LattConst_c = 779 pm (正交, <219 °C)
|   Coordination = 
  }}
| Section7 = {{Chembox Hazards
|   ExternalMSDS = [http://www.espimetals.com/msds's/cesiumperchlorate.pdf External MSDS]
|   EUIndex = 沒列明
|   FlashPt = 不可燃
  }}
| Section8 = {{Chembox Related
|   OtherAnions = [[氯化銫|氯化銫]]<br/>[[氯酸銫|氯酸銫]]
|   OtherCations = {{鹼金屬相關化學品|高氯酸}}
  }}
}}
'''高氯酸銫'''是一種[[無機化合物|無機化合物]]，屬於[[高氯酸鹽|高氯酸鹽]]，為一種[[鹽類|鹽類]]，其化學式為CsClO<sub>4</sub>。'''高氯酸銫'''能作氧化劑。'''高氯酸銫'''是白色吸濕性結晶，容易溶於[[水|水]]和[[乙醇|乙醇]]。

是可溶性最低的[[鹼金屬|鹼金屬]][[高氯酸鹽|高氯酸鹽]]（其次是[[銣|銣]]、[[鉀|鉀]]、[[鋰|鋰]]、和[[鈉|鈉]]），屬性可用於分液的目的，甚至重分析。<ref>{{Greenwood&Earnshaw1st|page=1017}}.</ref> 這種低溶解度發揮了重要作用於為鹼金屬鍅，如[[高氯酸鍅|高氯酸鍅]]與高氯酸銫<ref>{{citation | first = E. K. | last = Hyde | title = Radiochemical Methods for the Isolation of Element 87 (Francium) | year = 1952 | journal = J. Am. Chem. Soc. | volume = 74 | issue = 16 | pages = 4181–84 | doi = 10.1021/ja01136a066}}</ref>。

{| class="wikitable"
|+ 高氯酸銫在水中的溶解度<ref name="perry"/><ref name="chtas"/>
|-
! 溫度(°C)
| 0
| 8.5
| 14
| 25
| 40
| 50
| 60
| 70
| 99
|-
! 溶解度 (g / 100 ml)
| 0.8
| 0.91
| 1.91
| 1.974
| 3.694
| 5.47
| 7.30
| 9.79
| 28.57
|-
|}

加熱時，當溫度高於 250° 時，高氯酸銫會分解為[[氯化銫|氯化銫]]。像所有的高氯酸鹽，這是一種強氧化劑，可能發生劇烈反應的還原劑和有機物質，特別是在高溫下。

==參考文獻==
* [https://web.archive.org/web/20070929130014/http://caligula.bcs.deakin.edu.au/bcs_admin/msds/msds_docs/Cesium%20perchlorate.pdf Sigma-Aldrich MSDS]
{{reflist}}

{{銫化合物}}
{{高氯酸盐}}

[[Category:高氯酸鹽|Category:高氯酸鹽]]
[[Category:铯化合物|Category:铯化合物]]