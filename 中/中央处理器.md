...
[[File:Intel_Core_I7-920_Boxed_-_14.JPG|thumb]] [[Core_i7|Core i7]] CPU裝置於主機板上]]
[[File:AMD_Phenom_die.png|thumb]] Phenom Quad-Core結構攝影]]
'''中央处理器 '''（{{lang-en|'''C'''entral '''P'''rocessing '''U'''nit}}，[[缩写|缩写]]：'''CPU'''），是[[计算机|计算机]]的主要设备之一，功能主要是解释计算机{{tsl|ja|指令||指令}}以及处理计算机[[软件|软件]]中的[[数据|数据]]。计算机的可编程性主要是指对中央处理器的[[编程|编程]]。1970年代以前，中央处理器由多个独立单元构成，后来发展出由[[集成电路|集成电路]]制造的中央处理器，這些高度收縮的元件就是所謂的[[微处理器|微处理器]]，其中分出的中央处理器最為复杂的电路可以做成单一微小功能强大的单元。
...
早期的中央处理器通常是为大型及特定应用的计算机而定制。但是，这种昂贵的为特定应用定制CPU的方法很大程度上已经让位于开发便宜、标准化、适用于一个或多个目的的处理器类。这个标准化趋势始于由单个晶体管组成的[[大型机|大型机]]和[[微机|微机]]年代，随着[[集成电路|集成电路]]的出现而加速。IC使得更为复杂的中央处理器可以在很小的空间中设计和制造（在[[微米|微米]]的[[數量级|數量级]]）。中央处理器的标准化和小型化都使得这一类数字设备和電子零件在现代生活中的出现频率远远超过有限应用专用的计算机。现代微处理器出现在包括从[[汽车|汽车]]到[[手机|手机]]到儿童玩具在内的各种物品中。

* [[算術邏輯單元|运算器]]：[[算术|算术]]、[[逻辑|逻辑]]（部件：[[算术逻辑单元|算术逻辑单元]]、[[累加器|累加器]]、[[寄存器|寄存器组]]、[[路径转换器|路径转换器]]、[[数据总线|数据总线]]）
* [[控制器|控制器]]：[[复位|复位]]、[[使能|使能]]（部件：[[计数器|计数器]]、[[指令寄存器|指令寄存器]]、[[指令解码器|指令解码器]]、[[状态寄存器|状态寄存器]]、[[时钟发生器|时钟发生器]]、[[微操作信号发生器|微操作信号发生器]]）
...
=== 多核心 ===
{{main|多核心}}
多核心中央處理器是在中央處理器晶片或封裝中包含多個處理器核心，以偶數為核心數目較為常見，一般共用-{zh-cn:二级; zh-tw:第二級}-[[快取|快取]]。現今使用雙核心和四核心以上處理器的個人電腦已相當普遍。另也有少數三核心、六核心、八核心、十核心處理器等。

第一颗双核心处理器为IBM POWER4处理器，2012年IBM发布了最新8核心的POWER 7＋处理器，拥有80MB L3缓存/芯片。

== 性能 ==
CPU的性能和速度取決於时钟频率（一般以赫茲或千兆赫兹計算，即hz与Ghz）和每週期可處理的指令（IPC），兩者合併起來就是每秒可處理的指令（IPS）。<ref>"CPU Frequency". CPU World Glossary. CPU World. 25 March 2008. Retrieved 1 January 2010.</ref>
...
== 實際應用 ==
中央处理器大規模應用在[[個人電腦|個人電腦]]上，現今電腦可進入家庭。全因[[集成電路|集成電路]]的發展，令大小、性能以及價位等多個方面均有長足的進步。現今中央处理器價錢平宜，用戶可自行組裝個人電腦。主機板等主要電腦元件，均配合中央处理器設計。不同類型的中央处理器安裝到主機板上使用不同類型的插槽中（目前主流的插槽有LGA 1151和Socket AM4。當中英特爾的處理器使用LGA1151，超微的則使用Socket AM4）。現令中央處理器變得更省電，溫度更低。大多數IBM PC兼容機使用x86架構的处理器，他們主要由英特爾和超微兩家公司生産，此外[[威盛電子|威盛電子]]或者[[全美達|全美達]]也有參與中央处理器的生產。但與IBM PC兼容機不同，在2006年之前蘋果電腦所使用的處理器一直是IBM PowerPC RISC，之後的蘋果電腦開始轉而採用英特爾的處理器以及系統。可見中央处理器在現代電腦的重要地位。
...
* {{tsl|en|Cirrus Logic||Cirrus Logic（凌雲邏輯）}}
* [[Cyrix|Cyrix（賽瑞克斯）]]
* [[龙芯|龍芯]]（[[中国科学院|中國科學院]]）
* {{tsl|en|DEC||DEC（迪吉多）}}
...
* [http://www.sigma.me/2011/03/31/CMP-overview.html CMP簡要介紹] - 簡要的介紹了片上多核處理器，對常見的片上多核處理器採用的技術進行了綜述
* [http://www.gamezero.com/team-0/articles/math_magic/micro/index.html Processor Design: An Introduction] - 詳盡地介紹微处理器的设计，虽然有些資料未完成和落伍，但仍具參考價值。
* [http://computer.howstuffworks.com/microprocessor.htm 微处理器如何运作]
* [http://arstechnica.com/articles/paedia/cpu/pipelining-2.ars/2 Pipelining: An Overview] - CPU管線技術的簡介，由[[Ars_Technica|Ars Technica]]成员所写。
* [http://arstechnica.com/articles/paedia/cpu/simd.ars/ SIMD Architectures] - SIMD的介紹与說明，尤其著重于SIMD与桌上型计算机的关系。一样由[[Ars_Technica|Ars Technica]]所写。
;微处理器生產商
* [http://www.amd.com/ Advanced Micro Devices] - [[超威半导体|AMD]]，其中一个生產相容x86计算机的CPU生產商
* [http://www.arm.com/ ARM Ltd] - [[安謀國際科技|安謀國際科技]]，少数只授權其CPU设计而沒有自行制造的公司。嵌入式應用软件最常被[[ARM架构|ARM架构]]微处理器执行。
* [http://www.freescale.com/ Freescale Semiconductor] -（前身是[[摩托罗拉|Motorola]]的）[[飞思卡尔|飞思卡尔]]，设计数款嵌入装置以及[[系统芯片|SoC]] PowerPC处理器。
* [https://web.archive.org/web/20060314100538/http://www-03.ibm.com/chips/ IBM Microelectronics] - [[IBM|IBM]]的微電子分公司，设计出许多IBM {{tsl|en|IBM POWER||POWER}}与[[PowerPC|PowerPC]]，包括许多[[游戏机|-{zh-hans:游戏机; zh-hant:電視遊樂器;}-]]的CPU。
* [http://www.intel.com/ Intel Corp] - [[英特爾|英特爾]]，许多著名CPU的生產者，包括[[IA-32|IA-32]]、[[安腾|IA-64]]与[[XScale|XScale]]。也是许多用在他們自家CPU的週邊產品的制造者。
* [http://www.mips.com/ MIPS Technologies] - [[MIPS科技公司|MIPS科技公司]]，[[MIPS架构|MIPS架构]]的制造者，在[[精简指令集|RISC]]设计領域的先鋒。
* [http://www.sun.com/ Sun Microsystems] - [[升阳|升阳]]，[[SPARC|SPARC]]架构（RISC）的制造者。
...
* [http://www.transmeta.com/ Transmeta] - [[全美達|全美達]]，低電力[[X86|X86]]相容CPU的創造者，例如{{tsl|en|Crusoe||Crusoe}}与{{tsl|en|Efficeon||Efficeon}}。
* [http://www.via.com.tw/ VIA] - [[威盛|威盛]]，低功率[[X86|X86]]相容CPU的制造商，例如[[C3|C3]]。
