{{distinguish|硝基脲}}
{{chembox
| Verifiedfields = changed
| Watchedfields = changed
| verifiedrevid = 426796341
| ImageFile = Urea nitrate.png
| ImageName = Structural formulae of the ions in urea nitrate
| ImageFile1 = Urea-nitrate-3D-balls.png
| ImageSize1 = 220px
| ImageName1 = Ball-and-stick models of the ions in urea nitrate
| IUPACName = 
| OtherNames = 尿素硝酸盐
|Section1={{Chembox Identifiers
| CASNo_Ref = {{cascite|changed|??}}
| CASNo = 124-47-0
|  PubChem = 31295
|  ChemSpiderID_Ref = {{chemspidercite|changed|chemspider}}
| ChemSpiderID = 29035
|  SMILES = C(=O)(N)N.[N+](=O)(O)[O-]
|  InChI = 1/CH4N2O.HNO3/c2*2-1(3)4/h(H4,2,3,4);(H,2,3,4)
|  InChIKey = AYTGUZPQPXGYFS-UHFFFAOYAL
|  StdInChI_Ref = {{stdinchicite|changed|chemspider}}
| StdInChI = 1S/CH4N2O.HNO3/c2*2-1(3)4/h(H4,2,3,4);(H,2,3,4)
|  StdInChIKey_Ref = {{stdinchicite|changed|chemspider}}
| StdInChIKey = AYTGUZPQPXGYFS-UHFFFAOYSA-N
}}
|Section2={{Chembox Properties
| Formula = CH<sub>5</sub>N<sub>3</sub>O<sub>4</sub>
| MolarMass = 123.068 g/mol
| Appearance = 白色晶体<ref name=Tian>田德余 等. 含能材料及相关物手册. 国防工业出版社, 2011. pp 121. 200035 尿素硝酸盐</ref>
| Density = 1.69 g/cm<sup>3</sup>
| MeltingPt = 163℃（分解）
| BoilingPt = 
| Solubility = 难溶（冷水）
| SolubleOther = 可溶于热水和醇<ref name=Tian />}}
|Section3={{Chembox Hazards
| MainHazards = 刺激性
| FlashPt = 
| AutoignitionPt =
 }}
}}
'''硝酸脲'''是尿素形成的硝酸盐，化学式为CH<sub>5</sub>N<sub>3</sub>O<sub>4</sub>，是一种[[含能材料|含能材料]]。

==制备==
硝酸脲可由[[硝酸|硝酸]]和[[尿素|尿素]]的反应制得。
:(NH<sub>2</sub>)<sub>2</sub>CO (aq) + HNO<sub>3</sub> (aq) → (NH<sub>2</sub>)<sub>2</sub>COHNO<sub>3</sub> (s)

==参考文献==
{{reflist}}

==拓展阅读==
* {{cite journal |vauthors=Almog J, Burda G, Shloosh Y, Abramovich-Bar S, Wolf E, Tamiri T |title=Recovery and detection of urea nitrate in traces |journal=J. Forensic Sci. |volume=52 |issue=6 |pages=1284–90 |date=November 2007 |pmid=17868267 |doi=10.1111/j.1556-4029.2007.00551.x |url=}}
* {{cite journal |author=Mr.X |title=Improvised Urea Nitrate |journal=aware eZine Gamma |date=July 2008 |url=http://www.awarenetwork.org/etc/gamma/?x=5 |deadurl=yes |archiveurl=https://web.archive.org/web/20080920082234/http://www.awarenetwork.org/etc/gamma/?x=5 |archivedate=2008-09-20 }}

[[File:Urea_nitrate_(IV).jpg|thumb]]

{{脲}}

[[Category:脲|Category:脲]]
[[Category:硝酸盐|Category:硝酸盐]]