{{Infobox golfer
| name              = Andrew Loupe
| image             = 
| imagesize         = 
| caption           = 
| fullname          = Andrew Gravolet Loupe
| nickname          = 
| birth_date        = {{birth date and age|1988|11|22}}
| birth_place       = [[巴頓魯治_(路易西安納州)|巴頓魯治 (路易西安納州)]]
| death_date        = <!-- {{Death date and age|YYYY|MM|DD|1988|11|22}} -->
| death_place       = 
| height            = {{height|ft=6|in=1}}
| weight            = {{convert|180|lb|kg st|abbr=on}}
| nationality       = {{USA}}
| residence         = [[巴頓魯治_(路易西安納州)|巴頓魯治 (路易西安納州)]]
| spouse            = 
| partner           = 
| children          = 
| college           = [[路易斯安納州立大學|LSU]]
| yearpro           = 2011
| retired           = <!-- Year retired -->
| tour              = [[PGA巡迴賽|PGA巡迴賽]]
| extour            = [[Web.com巡迴賽|Web.com巡迴賽]]
| prowins           = 1
| pgawins           = 
| eurowins          = <!-- Number of European Tour wins -->
| japwins           = <!-- Number of Japan Golf Tour wins -->
| asiawins          = <!-- Number of Asian Tour wins -->
| sunwins           = <!-- Number of Sunshine Tour wins -->
| auswins           = <!-- Number of PGA Tour of Australasia wins -->
| nwidewins         = 1
| chalwins          = <!-- Number of Challenge Tour wins -->
| champwins         = <!-- Number of Champions Tour wins -->
| seneurowins       = <!-- Number of European Seniors Tour wins -->
| otherwins         = <!-- Number of Other wins -->
| majorwins         = <!-- Number of Major Championship wins -->
| masters           = 
| usopen            = 
| open              = 
| pga               = 
| wghofid           = <!-- World Golf Hall of Fame member ID -->
| wghofyear         = <!-- World Golf Hall of Fame year inducted -->
| award1            = 
| year1             = 
| awardssection     = <!-- location of awards page or section -->
}}
'''安德魯·魯普'''（'''Andrew Gravolet Loupe'''，{{bd|1988年|11月22日}}）是[[美國|美國]]的一位職業高爾夫球運動員，現在參加[[PGA巡迴賽|PGA巡迴賽]]的賽事。他出生在巴頓魯治，曾就讀於[[路易斯安那州立大學|路易斯安那州立大學]]並且是學校高爾夫球球隊的成員<ref name="pga">{{cite web |url=http://www.pgatour.com/players/player.35300.andrew-loupe.html/profile |title=Player Profile: Andrew Loupe |publisher=[[PGA_Tour|PGA Tour]] |accessdate=March 30, 2014}}</ref>。
==參考資料==
{{reflist}}
[[Category:美國高爾夫球運動員|Category:美國高爾夫球運動員]]