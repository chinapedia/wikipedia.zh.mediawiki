{{Infobox Settlement
 | name            = 派尼黑
 | official_name            = Пеники
 | settlement_type          = 村镇
| subdivision_name        = [[俄罗斯|俄罗斯]]
| subdivision_type1       = 州
| subdivision_name1       = [[列宁格勒州|列宁格勒州]]
| subdivision_type2       = 区
| subdivision_name2       = 罗蒙诺索夫区
| image_shield = Coat of arms of Penikovskoye (Peniki) Rural Settlement 2nd.png
| etymology = 词的起源 - [[芬兰语|芬兰语]]：“[[:en:wikt:penikka|penikka]]” = 小狗儿，狗崽子
 | pushpin_map              = Russia
 | pushpin_label_position   = bottom
 | pushpin_mapsize          = 280
 | pushpin_map_caption      = 地图
 | population_as_of         = 2010
 | population_total         = 1207
 | timezone                 = [[UTC+3|UTC+3]]
 | postal_code_type        = 邮政编码
 | postal_code             = 188530
 | latd = 59 | latm = 55 | lats = 03 | latNS = N
 | longd = 29 | longm = 38 | longs = 31 | longEW = E
}}

'''派尼黑''' ({{lang-ru|Пеники}}, {{IPA-ru|ˈpʲenʲɪkʲɪ}}；{{lang-fi|Penikkala}})，位于[[俄罗斯|俄罗斯]][[罗蒙诺索夫区|罗蒙诺索夫区]][[列宁格勒州|列宁格勒州]]，北临[[芬兰湾|芬兰湾]]，是一个村庄。
人口：1207（2010年）。邮政编码：188530。拨号代码：+7 8137654。

== 人口 ==
<timeline>
ImageSize = width:450 height:300
PlotArea = left:40 right:40 top:20 bottom:20
TimeAxis = orientation:vertical
AlignBars = justify
Colors =
  id:gray1 value:gray(0.9)
DateFormat = yyyy
Period = from:0 till:1335
ScaleMajor = unit:year increment:500 start:0 gridcolor:gray1
PlotData =
  bar:1838 color:gray1 width:1
   from:0 till:92 width:15  text:92 textcolor:red fontsize:8px
  bar:1848 color:gray1 width:1
   from:0 till:124 width:15  text:124 textcolor:red fontsize:8px
  bar:1862 color:gray1 width:1
   from:0 till:143 width:15 text:143 textcolor:red fontsize:8px
  bar:1885 color:gray1 width:1
   from:0 till:155 width:15  text:155 textcolor:red fontsize:8px
 bar:1928 color:gray1 width:1
  from:0 till:253 width:15 text:253 textcolor:red fontsize:8px
 bar:1965 color:gray1 width:1
  from:0 till:360 width:15 text:360 textcolor:red fontsize:8px
  bar:1990 color:gray1 width:1
   from:0 till:732 width:15  text:732 textcolor:red fontsize:8px
  bar:1997 color:gray1 width:1
   from:0 till:1309 width:15  text:1309 textcolor:red fontsize:8px
  bar:2002 color:gray1 width:1
  from:0 till:1007 width:15 text:1007 textcolor:red fontsize:8px
  bar:2007 color:gray1 width:1
   from:0 till:1335 width:15  text:1335 textcolor:red fontsize:8px
  bar:2010 color:gray1 width:1
   from:0 till:1207 width:15  text:1207 textcolor:red fontsize:8px
</timeline>

== 外部連結 ==
* {{Commons-inline|Peniki}}
* {{Official website|peniki47.ru}}
* [http://wikimapia.org/#lat=59.9159457&lon=29.6451473&z=16&l=66&m=b 地图 (Wikimapia)] (简体中文)
* [http://wikimapia.org/#lat=59.9159457&lon=29.6451473&z=16&l=6&m=b 地图 (Wikimapia)] (繁体中文)

[[Category:罗蒙诺索夫区|Category:罗蒙诺索夫区]]