{{Infobox mountain
| name = 肯尼亚山
| photo = Mount Kenya.jpg
| photo_caption = 肯尼亚山
| elevation_m = 5199
| elevation_ref = <ref name="peaklist">[http://peaklist.org/WWlists/ultras/africa.html "Africa Ultra-Prominences"] Peaklist.org. Retrieved 2012-02-06.</ref>
| prominence_m = 3825
| prominence_ref = <ref name="peaklist"/><br /><small>第32位</small>
| listing = [[各国最高点列表|全国最高点]]
| map = Kenya
| map_size = 250
| label_position = 
| location = [[肯尼亚|肯尼亚]]
| lat_d = 0 | lat_m = 9 | lat_s = 03 | lat_NS = S
| long_d = 37 | long_m = 18 | long_s = 27 | long_EW = E
| coordinates_ref = <ref name="peaklist"/>
| topo = 肯尼亚山（维洛霍夫斯基和萨维奇）<ref>{{cite web |url=http://www.ewpnet.com/maps/mkinside.htm |title=Mount Kenya Map Sample |publisher=Ewpnet.com |date= |accessdate=16 April 2010 |deadurl=yes |archiveurl=https://web.archive.org/web/20100821014732/http://www.ewpnet.com/maps/mkinside.htm |archivedate=2010年8月21日 }}</ref><ref name=map>{{cite map |publisher = EWP |title = Mount Kenya Map and Guide |url = http://www.ewpnet.com/Kenyamap.htm |edition = 4th |year = 2007 |cartography = EWP |scale = 1:50,000 with 1:25,000 inset |series = EWP Map Guides |isbn = 978-0-906227-96-1 |deadurl = yes |archiveurl = https://web.archive.org/web/20090227133844/http://www.ewpnet.com/Kenyamap.htm |archivedate = 2009-02-27 }}</ref>
| type = [[复式火山|复式火山]]（死火山）
| age =
| last_eruption = 2.6–3.1 MYA
| first_ascent = 1899年（[[哈尔福德·麦金德|哈尔福德·麦金德]]，随从向导塞萨尔·奥利耶和约瑟夫·布洛契尔）
| easiest_route = 攀岩
}}

'''肯尼亚山'''（{{lang-en|Mount Kenya}}）为[[肯尼亚|肯尼亚]]第一高峰及非洲第二高峰，仅次于[[乞力马扎罗山|乞力马扎罗山]]<ref name=rough_guide/>。最高主峰为巴蒂安峰（{{convert|5199|m|ft|0}}）、内利恩峰（{{convert|5188|m|ft|0}}）及勒纳纳峰（{{convert|4985|m|ft|0}}）。肯尼亚山位于肯尼亚中部，[[赤道|赤道]]以南约{{convert|16.5|km|mi|1}}，首都[[内罗毕|内罗毕]]东北偏北约{{convert|150|km|mi|0}}<ref name=rough_guide>{{cite map
| publisher=Rough Guide | title= Rough Guide Map Kenya
| edition=9 | year=2006 | cartography=World Mapping Project
| scale=1:900,000 | series=Rough Guide Map |isbn=1-84353-359-6
}}</ref>。肯尼亚共和国国名即来源于肯尼亚山。

肯尼亚山为一[[复式火山|复式火山]]，于三百万年前随[[东非大裂谷|东非裂谷带]]形成而产生<ref name=Rift>{{cite web
|url=http://tel.archives-ouvertes.fr/docs/00/15/90/18/PDF/These_P.Nonnotte_web.pdf
|author=Philippe Nonnotte
|title= ''Étude volcano-tectonique de la zone de divergence Nord-Tanzanienne (terminaison sud du rift kenyan)'' – Caractérisation pétrologique et géochimique du volcanisme récent (8 Ma – Actuel) et du manteau source – Contraintes de mise en place thèse de doctorat de l'université de Bretagne occidentale, spécialité : géosciences marines}}</ref>。冰期到来前其海拔为{{convert|7000|m|ft}}。过去数千年来其为[[冰帽|冰帽]]所覆盖，由此导致其极度[[侵蚀作用|侵蚀]]的山坡<ref name=gregory1894>
{{cite journal
 | last= Gregory
 | first= J. W.
 | authorlink = John Walter Gregory
 | year=1894
 | title = Contributions to the Geology of British East Africa.-Part I. The Glacial Geology of Mount Kenya
 | journal= Quarterly Journal of the Geological Society
 | volume=50
 | issue=
 | pages = 515–530
 | url = <!--  | accessdate=4 June 2007
 | doi = 10.1144/GSL.JGS.1894.050.01-04.36 -->
 | doi= 10.1144/GSL.JGS.1894.050.01-04.36
}}
</ref>及自中心发散的诸多峡谷<ref name=baker>{{cite book
 | last=Baker | first = B. H.
 | title = Geology of the Mount Kenya area | year=1967
 | publisher=Geological Survey of Kenya | location=Nairobi
 }}</ref>。目前肯尼亚山共有11座小型[[冰川|冰川]]。山坡植被覆盖率高，为肯尼亚提供了重要的水源<ref name=development>{{cite journal
 |last        = Gichuki
 |first       = Francis Ndegwa
 |title       = Threats and Opportunities for Mountain Area Development in Kenya
 |journal     = Ambio
 |volume      = 28
 |issue       = 5
 |pages       = 430–435
 |publisher   = Royal Swedish Academy of Sciences
 |date        = August 1999
 |url         = http://www.ambio.kva.se
 |deadurl     = yes
 |archiveurl  = https://web.archive.org/web/20051231095818/http://www.ambio.kva.se/
 |archivedate = 2005-12-31
}}
</ref>。

自山脚至山顶有诸多[[生物群系|植被带]]<ref name=kirinyaga>{{cite book
|title= Kirinyaga: a fable of Utopia
|last= Resnick
|first= Mike
|authorlink= Mike Resnick
|year= 1998
|publisher= Ballantine
|location=
|isbn= 0-345-41701-1
|page= 293
|url=
|accessdate= }}
</ref>，山坡下部由多种树林覆盖。诸多高山植物为肯尼亚[[特有种|特有]]，如巨型[[半边莲属|半边莲]]、[[黄菀属|黄菀]]及当地亚种[[蹄兔|蹄兔]]<ref name=ecology>{{cite book
| last=Coe | first=Malcolm James
| title=The Ecology of the Alpine Zone of Mount Kenya
| year=1967 | publisher= Dr W. Junk | location=The Hague}}</ref>。肯尼亚山中部面积{{convert|715|km2|sqmi}}的地区被指定为[[肯尼亚山国家公园|国家公园]]<ref name=uicn>{{cite web|url=http://whc.unesco.org/archive/advisory_body_evaluation/800.pdf
| title= World Heritage Nomination – IUCN Technical Evaluation Mount Kenya (Kenya)}}</ref>，于1997年成为[[世界遗产|联合国教科文组织世界遗产]]<ref name=unesco>{{cite web
 |url         = http://whc.unesco.org/pg.cfm?cid=31&id_site=800
 |title       = Mount Kenya National Park/Natural Forest
 |accessdate  = 23 February 2008
 |author      = United Nations
 |authorlink  = United Nations
 |year        = 2008
 |archiveurl  = https://web.archive.org/web/20061230202343/http://whc.unesco.org/pg.cfm?cid=31&id_site=800
 |archivedate = 2006年12月30日
 |deadurl     = yes
}}</ref>。国家公园每年访客量超过16,000人<ref name=development/>。

{{Authority control}}
[[Category:肯亞地理|Category:肯亞地理]]
[[Category:肯雅山峰|Category:肯雅山峰]]